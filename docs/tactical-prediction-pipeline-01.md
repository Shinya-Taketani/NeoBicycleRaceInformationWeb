...
- 依頼時刻はUTC・マイクロ秒精度で比較する。旧固定束のローカル時刻も比較時だけ正規化し、request.json/manifest/LOCKEDは書き換えない。
...
- 出力rootのパス検証とディレクトリ準備を分離し、モデル参照元・その配下との重複は準備前に拒否する。拒否時に参照元へ空ディレクトリやイベントを追加しない。
- 再現成功時はイベントに予測hashとstage名を保存した後、その呼出しが作成した2ファイルとstageだけを削除する。不一致・イベント保存失敗時のstage、過去のstage/失敗証拠/固定束は残す。
...
対象HEAD `2f2fa3ca14553bee36476622a88577ece75e2cca` 上で時刻同値比較、書込み前の参照元重複検査、成功再現stageの限定削除を修正。solver/モデル/入力定義は変更していない。
追加15ケースを含む接続48テストは128MBで成功（926 assertions）。全体は1,155成功・PostgreSQL専用9skip・8,754 assertions。変更PHPのPintと構文検査、git diff --checkは成功。全体Pintの未変更E08テストの既存指摘は残す。

実データは固定済みレース37750の入力・モデルだけで、DB接続を無効にして確認した。
`2025-12-31 22:07:00+09` と `2025-12-31T13:07:00Z` はどちらもREUSED。旧requestの `2025-12-31T22:07:00.000000+09:00` 表記は不変。
再現予測は1行・8,732 bytes・SHA-256 `0c91b61a1ec939799844bbcbd3ebbc1088330489df5996ae8227a110ed367f4c` で既存と完全一致。最終モデルhashも不変。
今回の成功stageだけが削除され、新規監査イベント3件を保持。既存6,741ファイル/ディレクトリと原本参照8ファイルは不変だった。
記録は既存保存先内の `pr57-review-02/result.json`。初回の外部検証スクリプトはCLI JSONの0.0/0・1.0/1の型差を差分と判定して停止したため、検証を原本hashと依頼/予測identityの比較に修正した。先のログは `pr57-review-01/` に残し、本番の数値処理・元成果物は変更していない。
