...
return new class extends Migration
{
    public function up(): void
    {
        ...
        if (DB::getDriverName() === 'pgsql') {
            ...
            DB::statement("ALTER TABLE backtest_bin_effects ADD CONSTRAINT bt_bin_effects_origin_shape_check CHECK ((bin_origin = 'TRAINING_BIN' AND source_backtest_effect_bin_id IS NOT NULL AND bin_index > 0) OR (bin_origin = 'UNSEEN_CATEGORY' AND source_backtest_effect_bin_id IS NULL AND bin_index = 0 AND training_sample_count = 0 AND bin_kind = 'CATEGORY'))");
            ...
            DB::statement("ALTER TABLE backtest_bin_effects ADD CONSTRAINT bt_bin_effects_observation_check CHECK ((evaluation_status = 'OBSERVED' AND evaluation_sample_count > 0 AND evaluation_race_count > 0 AND evaluation_race_count <= evaluation_sample_count AND positive_count <= evaluation_sample_count) OR (evaluation_status = 'NO_EVALUATION_ROWS' AND evaluation_sample_count = 0 AND evaluation_race_count = 0 AND positive_count = 0))");
            DB::statement('ALTER TABLE backtest_bin_effects ADD CONSTRAINT bt_bin_effects_bootstrap_check CHECK (bootstrap_iterations > 0)');
