...
use App\Domain\Keirin\Statistics\DTO\Batch03BuildSummaryDto;
use App\Domain\Keirin\Statistics\DTO\Batch03FeatureResultDto;
use App\Domain\Keirin\Statistics\DTO\Batch03StatSummaryDto;
...
class Batch03FeatureBuildService
{
    private const SUBJECT_TYPE = 'RACE_ENTRY';

    private const INPUT_AS_OF_POLICY = 'STAT01_RUN_RESULT_INPUT_AS_OF';

    /** @var array<string, Batch03Calculator> */
    private array $calculators;

    public function __construct(
        private readonly Batch03HistoricalRaceRepository $inputs,
        Stat07Calculator $stat07,
        Stat08Calculator $stat08,
        Stat23Calculator $stat23,
        Stat31Calculator $stat31,
        Stat32Calculator $stat32,
        Stat33Calculator $stat33,
    ) {
        $this->calculators = [];
        foreach ([$stat07, $stat08, $stat23, $stat31, $stat32, $stat33] as $calculator) {
            $this->calculators[$calculator->stat()->value] = $calculator;
        }
    }

    public function build(Batch03BuildOptionsDto $options): Batch03BuildSummaryDto
    {
        ...
        $counters = [];
        foreach (Batch03Stat::cases() as $stat) {
        ...
        foreach (Batch03Stat::cases() as $stat) {
            ...
            $summaries[] = new Batch03StatSummaryDto(
                statCode: $stat->value,
        ...
        return new Batch03BuildSummaryDto($batchExecutionUuid, $options->dryRun, $targets->races, $targets->entries, $summaries);
    }

    /** @return array<string, StatisticFeatureRun> */
    private function startRuns(Batch03BuildOptionsDto $options, string $batchExecutionUuid, int $targetRaces, int $targetEntries): array
    {
    ...
    private function storeResult(StatisticFeatureRun $run, int $raceId, Batch03FeatureResultDto $result, DateTimeImmutable $calculatedAt): void
    {
    ...
    /** @param list<Batch03FeatureResultDto> $results */
