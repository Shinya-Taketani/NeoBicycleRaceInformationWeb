...
final class Objective
{
    ...
    /** @param list<float> $coefficients */
    public function smoothPenalty(Layout $layout, array $coefficients, float $lambda): float
    {
        $size = $layout->size();
    ...
    public function smoothPenaltyGradient(Layout $layout, array $coefficients, float $lambda): array
    {
    ...
    /** @param list<float> $coefficients */
    public function groupPenalty(Layout $layout, array $coefficients, float $lambda): float
    {
     ...
     */
    private function eligibleEntries(array $race, Layout $layout, int $targetRank): ?array
    {
    ...
    /** @param list<float> $coefficients */
    private function targetRank(string $position, array $coefficients, Layout $layout): int
    {
    ...
    private function assertEntry(array $entry, Layout $layout): void
    {
        if ((! is_float($entry['anchor'] ?? null) && ! is_int($entry['anchor'] ?? null))
            ...
            || count($entry['bins']) !== $layout->featureCount()
