...
final class Bt03e06RaceSpool
{
    private const FORMAT_VERSION = 'BT03E06-RACE-SPOOL-v1';
    ...
    {
        if (! in_array($role, ['RECONSTRUCTED', 'DECODER', 'CONTEXT'], true)) {
            throw new RuntimeException('BT-03E-06 spool role was invalid.');
        }
        $this->handle = fopen($path, 'xb');
        if ($this->handle === false) {
            throw new RuntimeException('Could not create the BT-03E-06 race spool.');
        }
    ...
    public function append(array $race): void
    {
        if ($this->handle === null || $this->metadata !== null
            || ! in_array($race['year'] ?? null, Bt03e06Contract::DEVELOPMENT_YEARS, true)
            || ! is_int($race['race_id'] ?? null) || $race['race_id'] < 1) {
            throw new RuntimeException('BT-03E-06 spool race identity was invalid.');
        }
        if ($this->role !== 'DECODER' && (! is_array($race['entries'] ?? null) || $race['entries'] === [])) {
            throw new RuntimeException('BT-03E-06 reconstructed/context spool race entries were empty.');
        }
        $line = json_encode([
    ...
    public function seal(): array
    {
        ...
        if ($this->handle === null || ! fflush($this->handle) || (function_exists('fsync') && ! fsync($this->handle))) {
            throw new RuntimeException('Could not seal the BT-03E-06 race spool.');
        }
        fclose($this->handle);
        $this->handle = null;
    ...
    {
        $metadata = $this->metadata ?? throw new RuntimeException('BT-03E-06 spool must be sealed before replay.');
        $handle = fopen($this->path, 'rb');
        if ($handle === false) {
            throw new RuntimeException('Could not replay the BT-03E-06 race spool.');
        }
        ...
        try {
            while (($line = fgets($handle)) !== false) {
                if (! str_ends_with($line, "\n")) {
                    throw new RuntimeException('BT-03E-06 spool row was incomplete.');
                }
                ...
                if (! is_array($row) || ($row['v'] ?? null) !== self::FORMAT_VERSION
                    ...
                    || ! in_array($row['year'] ?? null, Bt03e06Contract::DEVELOPMENT_YEARS, true)
                    || ! is_int($row['race_id'] ?? null)) {
                    throw new RuntimeException('BT-03E-06 spool replay contract was invalid.');
                }
            ...
            if (! feof($handle) || $races !== $metadata['race_count'] || $entries !== $metadata['entry_count']
                ...
                throw new RuntimeException('BT-03E-06 spool replay did not match its seal.');
            }
        } finally {
            fclose($handle);
        }
    ...
    {
        return $this->metadata ?? throw new RuntimeException('BT-03E-06 spool was not sealed.');
    }
    ...
    private function write(string $bytes): void
    {
        ...
        while ($offset < strlen($bytes)) {
            ...
            if ($written === false || $written === 0) {
                throw new RuntimeException('Could not write the BT-03E-06 race spool.');
            }
            $offset += $written;
        }
    }
