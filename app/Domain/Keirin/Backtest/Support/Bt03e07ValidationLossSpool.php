...
final class Bt03e07ValidationLossSpool
{
    private const VALUE_COUNT = 16;
    ...
    {
        $canonical = array_map(self::lambdaKey(...), Bt03e07Contract::LAMBDA_GRID);
        ...
        if (count($availableLambdaKeys) !== count(array_unique($availableLambdaKeys))
            ...
            throw new RuntimeException('BT-03E-07 validation loss candidates were invalid.');
        }
        ...
        if ($this->handle === false) {
            throw new RuntimeException('Could not create the BT-03E-07 validation loss spool.');
        }
    }
    ...
    public function append(array $losses): void
    {
        if ($this->handle === null || $this->sealed) {
            throw new RuntimeException('BT-03E-07 validation loss spool was not writable.');
        }
        $values = [];
        foreach (Bt03e07Contract::LAMBDA_GRID as $lambda) {
            ...
            foreach (Bt03e07Contract::POSITIONS as $position) {
                ...
                if ($value !== null && ! is_finite($value)) {
                    throw new RuntimeException('BT-03E-07 validation loss was non-finite.');
                }
                $values[] = $value ?? NAN;
            }
        }
        if (count($values) !== self::VALUE_COUNT) {
            throw new RuntimeException('BT-03E-07 validation loss vector was invalid.');
        }
    ...
    public function seal(): void
    {
        ...
        if ($this->handle === null || ! fflush($this->handle) || (function_exists('fsync') && ! fsync($this->handle))) {
            throw new RuntimeException('Could not seal the BT-03E-07 validation loss spool.');
        }
        fclose($this->handle);
        $this->handle = null;
        $this->sealed = true;
    ...
    {
        if (! $this->sealed) {
            throw new RuntimeException('BT-03E-07 validation loss spool was not sealed.');
        }
        $handle = fopen($this->path, 'rb');
        if ($handle === false) {
            throw new RuntimeException('Could not replay the BT-03E-07 validation loss spool.');
        }
        $count = 0;
        try {
            while (($record = fread($handle, self::RECORD_BYTES)) !== '') {
                if (strlen($record) !== self::RECORD_BYTES) {
                    throw new RuntimeException('BT-03E-07 validation loss record was incomplete.');
                }
                $values = unpack('E*', $record);
                if ($values === false || count($values) !== self::VALUE_COUNT) {
                    throw new RuntimeException('BT-03E-07 validation loss record was invalid.');
                }
                $count++;
            ...
            if (! feof($handle) || $count !== $this->raceCount) {
                throw new RuntimeException('BT-03E-07 validation loss count drifted.');
            }
        } finally {
            fclose($handle);
        }
    ...
    private function write(string $bytes): void
    {
        ...
        while ($offset < strlen($bytes)) {
            ...
            if ($written === false || $written === 0) {
                throw new RuntimeException('Could not write the BT-03E-07 validation loss spool.');
            }
            $offset += $written;
        }
    }
