...
final class Bt03e08ValidationLossSpool
{
    private const VALUE_COUNT = 8;
    ...
    {
        $canonical = array_map(self::lambdaKey(...), Bt03e08Contract::LAMBDA_GRID);
        ...
        if (count($availableLambdaKeys) !== count(array_unique($availableLambdaKeys)) || array_diff($availableLambdaKeys, $canonical) !== []) {
            throw new RuntimeException('BT-03E-08 validation loss candidates were invalid.');
        }
        $this->availableLambdaKeys = array_values(array_filter($canonical, static fn (string $key): bool => in_array($key, $availableLambdaKeys, true)));
        $this->handle = fopen($path, 'xb');
        if ($this->handle === false) {
            throw new RuntimeException('Could not create the BT-03E-08 validation loss spool.');
        }
    }
    ...
    public function append(array $losses): void
    {
        if ($this->handle === null || $this->sealed) {
            throw new RuntimeException('BT-03E-08 validation loss spool was not writable.');
        }
        $values = [];
        foreach (Bt03e08Contract::LAMBDA_GRID as $lambda) {
            $key = self::lambdaKey($lambda);
            $value = in_array($key, $this->availableLambdaKeys, true) ? ($losses[$key] ?? null) : null;
            if ($value !== null && ! is_finite($value)) {
                throw new RuntimeException('BT-03E-08 validation loss was non-finite.');
            }
            $values[] = $value ?? NAN;
        }
    ...
    public function seal(): void
    {
        ...
        if ($this->handle === null || ! fflush($this->handle) || (function_exists('fsync') && ! fsync($this->handle))) {
            throw new RuntimeException('Could not seal the BT-03E-08 validation loss spool.');
        }
        fclose($this->handle);
        $this->handle = null;
        $this->sealed = true;
    ...
    {
        if (! $this->sealed) {
            throw new RuntimeException('BT-03E-08 validation loss spool was not sealed.');
        }
        $handle = fopen($this->path, 'rb');
        if ($handle === false) {
            throw new RuntimeException('Could not replay the BT-03E-08 validation loss spool.');
        }
        $count = 0;
        try {
            while (($record = fread($handle, self::RECORD_BYTES)) !== '') {
                if (strlen($record) !== self::RECORD_BYTES) {
                    throw new RuntimeException('BT-03E-08 validation loss record was incomplete.');
                }
                $values = unpack('E*', $record);
                if ($values === false || count($values) !== self::VALUE_COUNT) {
                    throw new RuntimeException('BT-03E-08 validation loss record was invalid.');
                }
                $count++;
            ...
            if (! feof($handle) || $count !== $this->raceCount) {
                throw new RuntimeException('BT-03E-08 validation loss count drifted.');
            }
        } finally {
            fclose($handle);
        }
    ...
    private function write(string $bytes): void
    {
        ...
        while ($offset < strlen($bytes)) {
            ...
            if ($written === false || $written === 0) {
                throw new RuntimeException('Could not write the BT-03E-08 validation loss spool.');
            }
            $offset += $written;
        }
    }
