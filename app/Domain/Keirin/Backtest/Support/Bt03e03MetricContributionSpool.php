...
final class Bt03e03MetricContributionSpool
{
    ...
    public function __construct(private readonly string $path)
    {
        ...
        if ($this->handle === false) {
            throw new RuntimeException('Could not create the BT-03E-03 metric contribution spool.');
        }
    }
    ...
    public function append(array $comparison): void
    {
        if ($this->handle === null || $this->sealed) {
            throw new RuntimeException('BT-03E-03 metric contribution spool was not writable.');
        }
        $values = [];
        ...
        if (count($values) !== self::VALUES_PER_RACE || array_filter($values, 'is_finite') !== $values) {
            throw new RuntimeException('BT-03E-03 metric contribution was invalid.');
        }
    ...
    public function seal(): void
    {
        ...
        if ($this->handle === null || ! fflush($this->handle) || (function_exists('fsync') && ! fsync($this->handle))) {
            throw new RuntimeException('Could not seal the BT-03E-03 metric contribution spool.');
        }
        fclose($this->handle);
        $this->handle = null;
        $this->sealed = true;
    ...
    {
        if (! $this->sealed) {
            throw new RuntimeException('BT-03E-03 metric contribution spool was not sealed.');
        }
        $handle = fopen($this->path, 'rb');
        if ($handle === false) {
            throw new RuntimeException('Could not replay the BT-03E-03 metric contribution spool.');
        }
        $count = 0;
        try {
            while (($record = fread($handle, self::RECORD_BYTES)) !== '') {
                if (strlen($record) !== self::RECORD_BYTES) {
                    throw new RuntimeException('BT-03E-03 metric contribution record was incomplete.');
                }
                $values = unpack('E*', $record);
                if ($values === false || count($values) !== self::VALUES_PER_RACE) {
                    throw new RuntimeException('BT-03E-03 metric contribution record was invalid.');
                }
                $count++;
                yield array_values($values);
            }
            if (! feof($handle) || $count !== $this->raceCount) {
                throw new RuntimeException('BT-03E-03 metric contribution count drifted.');
            }
        } finally {
            fclose($handle);
        }
    ...
    private function write(string $bytes): void
    {
        ...
        while ($offset < strlen($bytes)) {
            ...
            if ($written === false || $written === 0) {
                throw new RuntimeException('Could not write the BT-03E-03 metric contribution spool.');
            }
            $offset += $written;
        }
    }
