...
final class Bt03e04MetricEvaluator
{
    ...
    public function emptySummary(): array
    {
        return [
            ...
            'diagnostic_counts' => [
                'ARGMAX_P1_WINNER_HIT_AT_1' => 0.0,
                'PRIMARY_COHERENT_WINNER_HIT_AT_1' => 0.0,
                'ARGMAX_P1_PRIMARY_FIRST_DISAGREEMENT' => 0.0,
                'PRIMARY_COHERENT_EXACT_ORDERED_TOP3_RATE' => 0.0,
    ...
    public function finish(array $summary): array
    {
        if ($summary['race_count'] < 1) {
            throw new RuntimeException('BT-03E-04 metric evaluation had no races.');
        }
        ...
        return [
            ...
            'decoder_diagnostics' => [
                'ARGMAX_P1_WINNER_HIT_AT_1' => $winnerEligible > 0 ? $diagnostics['ARGMAX_P1_WINNER_HIT_AT_1'] / $winnerEligible : 0.0,
                'PRIMARY_COHERENT_WINNER_HIT_AT_1' => $winnerEligible > 0 ? $diagnostics['PRIMARY_COHERENT_WINNER_HIT_AT_1'] / $winnerEligible : 0.0,
                'ARGMAX_P1_PRIMARY_FIRST_DISAGREEMENT_RATE' => $diagnostics['ARGMAX_P1_PRIMARY_FIRST_DISAGREEMENT'] / $summary['race_count'],
                'PRIMARY_COHERENT_EXACT_ORDERED_TOP3_RATE' => $orderedEligible > 0 ? $diagnostics['PRIMARY_COHERENT_EXACT_ORDERED_TOP3_RATE'] / $orderedEligible : 0.0,
    ...
    public function raceComparison(array $context, array $decision): array
    {
        if (($context['year'] ?? null) !== ($decision['year'] ?? null)
            ...
            throw new RuntimeException('BT-03E-04 metric context and decision identities differed.');
        }
        ...
        return [
            ...
            'ties' => [
                'primary_score_tied_races' => (int) ($decision['primary_tie_count'] > 1),
                'primary_score_tied_combinations' => $decision['primary_tie_count'] > 1 ? $decision['primary_tie_count'] : 0,
                'technical_tiebreak_races' => (int) $decision['primary_technical_tiebreak_used'],
                ...$this->baselineTies($baselineEntries),
            ],
            'diagnostics' => [
                'ARGMAX_P1_WINNER_HIT_AT_1' => (float) ($winnerEligible && $decision['argmax_p1_bike'] === $winner),
                'PRIMARY_COHERENT_WINNER_HIT_AT_1' => (float) ($winnerEligible && $primary[0] === $winner),
                'ARGMAX_P1_PRIMARY_FIRST_DISAGREEMENT' => (float) ($decision['argmax_p1_bike'] !== $primary[0]),
                'PRIMARY_COHERENT_EXACT_ORDERED_TOP3_RATE' => (float) ($ordered !== null && $primary === $ordered),
