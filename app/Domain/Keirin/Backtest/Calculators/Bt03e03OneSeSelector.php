...
final class Bt03e03OneSeSelector
{
    /**
     * @param  array<int,Bt03e03ValidationLossSpool>  $losses
     ...
     */
    public function select(array $losses, int $iterations = Bt03e03Contract::BOOTSTRAP_ITERATIONS): array
    {
        if ($losses === [] || $iterations < 2) {
            throw new RuntimeException('BT-03E-03 One-SE input was empty or invalid.');
        }
        ...
        if ($available === []) {
            throw new RuntimeException('BT-03E-03 One-SE had no fully converged shared lambda candidate.');
        }
        ...
        for ($iteration = 0; $iteration < $iterations; $iteration++) {
            ...
            foreach ($losses as $spool) {
                ...
                if ($count < 1) {
                    throw new RuntimeException('BT-03E-03 One-SE year had no races.');
                }
    ...
    /** @param array<int,Bt03e03ValidationLossSpool> $years @param list<string> $keys @return array<string,float> */
    ...
    private function weightedAggregates(Bt03e03ValidationLossSpool $spool, array $weights, array $keys): array
    {
        $offsets = array_flip(array_map(Bt03e03ValidationLossSpool::lambdaKey(...), Bt03e03Contract::LAMBDA_GRID));
        $sums = $counts = [];
        foreach ($keys as $key) {
            $sums[$key] = array_fill_keys(Bt03e03Contract::POSITIONS, 0.0);
            $counts[$key] = array_fill_keys(Bt03e03Contract::POSITIONS, 0);
        }
        $raceIndex = 0;
        foreach ($spool->records() as $values) {
            $weight = $weights[$raceIndex++] ?? throw new RuntimeException('BT-03E-03 One-SE weight count drifted.');
            if ($weight === 0) {
                continue;
            }
            foreach ($keys as $key) {
                $lambdaOffset = $offsets[$key] ?? throw new RuntimeException('BT-03E-03 One-SE lambda offset was invalid.');
                foreach (Bt03e03Contract::POSITIONS as $positionOffset => $position) {
                    $loss = $values[$lambdaOffset * count(Bt03e03Contract::POSITIONS) + $positionOffset];
                    if ($loss !== null) {
        ...
        if ($raceIndex !== count($weights)) {
            throw new RuntimeException('BT-03E-03 One-SE loss count drifted.');
        }
        $aggregates = [];
        foreach ($keys as $key) {
            $positionMeans = [];
            foreach (Bt03e03Contract::POSITIONS as $position) {
                if ($counts[$key][$position] === 0) {
                    throw new RuntimeException("BT-03E-03 {$position} bootstrap sample had no eligible races.");
                }
