...
final class Bt03e07FistaOptimizer
{
    public function __construct(private readonly Bt03e07DirectPositionObjective $objective) {}

    /**
     * @param  callable():iterable<array<string,mixed>>  $raceSource
     * @return array{fits:array<string,Bt03e07FitResultDto>,candidate_statuses:array<string,array<string,mixed>>,fit_order:list<float>}
     */
    public function fitPath(callable $raceSource, Bt03e02ParameterLayout $layout): array
    {
        $path = $this->runPath($raceSource, $layout);
        if (count($path['candidate_statuses']) !== count(Bt03e07Contract::LAMBDA_GRID)) {
            throw new RuntimeException('BT-03E-07 full lambda path did not audit every fixed candidate.');
        }
        if ($path['fits'] === []) {
            throw new RuntimeException('BT-03E-07 all fixed lambda candidates were numerically non-converged.');
        }
    ...
    /**
     * @param  callable():iterable<array<string,mixed>>  $raceSource
     * @return array{fit:Bt03e07FitResultDto,selected_lambda:float,candidate_statuses:array<string,array<string,mixed>>,fit_order:list<float>}
     */
    public function fitSelectedViaPath(
        ...
        if (! in_array($selectedLambda, Bt03e07Contract::LAMBDA_GRID, true)) {
            throw new RuntimeException('BT-03E-07 selected lambda was outside the frozen grid.');
        }
        $path = $this->runPath($raceSource, $layout, $selectedLambda);
        $key = $this->lambdaKey($selectedLambda);
        if (! isset($path['fits'][$key])) {
            throw new RuntimeException(
                sprintf('BT-03E-07 selected lambda %.17g did not converge during path refit; fallback was forbidden.', $selectedLambda),
    ...
    /**
     * @param  callable():iterable<array<string,mixed>>  $raceSource
     * @return array{fits:array<string,Bt03e07FitResultDto>,candidate_statuses:array<string,array<string,mixed>>,fit_order:list<float>,failures:array<string,Bt03e07OptimizerNonConvergenceException>}
     */
    private function runPath(
        ...
        foreach (Bt03e07Contract::FIT_EXECUTION_ORDER as $lambda) {
            $fitOrder[] = $lambda;
        ...
        $fits = $statuses = [];
        foreach (Bt03e07Contract::LAMBDA_GRID as $lambda) {
            $key = $this->lambdaKey($lambda);
    ...
    public function fit(
        ...
        if (! in_array($lambda, Bt03e07Contract::LAMBDA_GRID, true)) {
            throw new RuntimeException('BT-03E-07 lambda was outside the frozen grid.');
        }
        ...
        return new Bt03e07FitResultDto($lambda, $coefficients, $objectives, $iterations, $eligible, $excluded, $diagnostics);
    }
    ...
    private function fitPosition(
        ...
        $accelerated = $current;
        $momentum = 1.0;
        $step = Bt03e07Contract::INITIAL_STEP;
        ...
        if (! is_finite($previousObjective)) {
            throw new RuntimeException("BT-03E-07 {$position} optimizer initial objective was non-finite.");
        }
        ...
        for ($iteration = 1; $iteration <= Bt03e07Contract::MAX_ITERATIONS; $iteration++) {
            $attemptsWithinUpdate = 0;
            while (true) {
                $attemptsWithinUpdate++;
                if ($attemptsWithinUpdate > 2) {
                    throw new RuntimeException("BT-03E-07 {$position} monotone restart retry invariant failed.");
                }
                $optimizerAttemptCount++;
                $retryingAfterRestart = false;
                if ($pendingRestartStep !== null) {
                    if ($step !== $pendingRestartStep) {
                        throw new RuntimeException("BT-03E-07 {$position} monotone restart step was not retained.");
                    }
                    $restartStepRetentionCount++;
                    $lastPostRestartIterationStartStep = $step;
                    $pendingRestartStep = null;
                ...
                $trialStep = $step;
                for ($lineSearch = 0; $lineSearch <= Bt03e07Contract::MAX_LINE_SEARCH_STEPS; $lineSearch++) {
                    $trial = [];
                    ...
                    $trialStep *= Bt03e07Contract::BACKTRACK_FACTOR;
                }
                if ($accepted === null || $acceptedLoss === null) {
                    throw new RuntimeException("BT-03E-07 {$position} FISTA line search failed.");
                }
                if ($lineSearch > 0) {
                    $backtrackingIterationCount++;
                }
                ...
                if (! is_finite($acceptedObjective)) {
                    throw new RuntimeException("BT-03E-07 {$position} optimizer produced a non-finite objective.");
                }
                ...
                if ($acceptedUpdateCount !== $iteration) {
                    throw new RuntimeException("BT-03E-07 {$position} accepted update accounting drifted.");
                }
                ...
                if ($maximumChange <= Bt03e07Contract::CONVERGENCE_TOLERANCE
                    && $relativeObjective <= Bt03e07Contract::OBJECTIVE_TOLERANCE) {
        ...
        throw new Bt03e07OptimizerNonConvergenceException($lastDiagnostics);
    }
    ...
    private function diagnostics(
        ...
        $diagnostics = [
            ...
            'max_iterations' => Bt03e07Contract::MAX_ITERATIONS,
        ...
        foreach ($diagnostics as $value) {
            if (is_float($value) && ! is_finite($value)) {
                throw new RuntimeException("BT-03E-07 {$position} optimizer diagnostics were non-finite.");
            }
        }
