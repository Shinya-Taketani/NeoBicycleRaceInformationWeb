...
final class Bt03e04AcceptanceGate
{
    ...
    public function evaluate(array $outer, array $intervals, bool $integrity): array
    {
        ...
        foreach (self::PRIMARY as $metric) {
            $nonInferiority = $nonInferiority
                && ($intervals[$metric]['ci_lower'] ?? -INF) > Bt03e04Contract::NON_INFERIORITY_CI_LOWER_THRESHOLD;
        }
        $yearEqual = [];
        ...
        $superiority = ($intervals['POSITION_HIT_RATE_AT_3']['ci_lower'] ?? -INF) > Bt03e04Contract::SUPERIORITY_CI_LOWER_THRESHOLD
            && count(array_filter(
                ['WINNER_HIT_AT_1', 'POSITION_2_ACCURACY', 'POSITION_3_ACCURACY'],
                static fn (string $metric): bool => ($intervals[$metric]['ci_lower'] ?? -INF) > Bt03e04Contract::SUPERIORITY_CI_LOWER_THRESHOLD,
            )) >= Bt03e04Contract::SUPERIORITY_POSITION_CI_POSITIVE_MIN_COUNT
            && count(array_filter(
                self::PRIMARY,
                static fn (string $metric): bool => $yearEqual[$metric] > Bt03e04Contract::SUPERIORITY_CI_LOWER_THRESHOLD,
            )) >= Bt03e04Contract::SUPERIORITY_PRIMARY_POSITIVE_MIN_COUNT;
        $temporal = true;
        foreach ($outer as $result) {
            foreach (self::PRIMARY as $metric) {
                $temporal = $temporal && $result['delta'][$metric] >= Bt03e04Contract::TEMPORAL_STABILITY_DELTA_THRESHOLD;
            }
        }
        $supporting = count(array_filter(
            self::SUPPORTING,
            static fn (string $metric): bool => $yearEqual[$metric] >= Bt03e04Contract::SUPPORTING_NON_NEGATIVE_THRESHOLD,
        )) >= Bt03e04Contract::SUPPORTING_MIN_NON_NEGATIVE_COUNT
            && count(array_filter(
                self::SUPPORTING,
                static fn (string $metric): bool => $yearEqual[$metric] < Bt03e04Contract::SUPPORTING_MIN_ALLOWED_DELTA,
            )) === 0;
        ...
        $tieQuality = $races > 0 && $candidateTies / $races <= $baselineTies / $races
            && $technicalTies / $races <= Bt03e04Contract::TECHNICAL_TIE_RATE_MAX;
        $positionRedesign = $yearEqual['WINNER_HIT_AT_1'] > Bt03e04Contract::POSITION_REDESIGN_WIN_MIN_EXCLUSIVE
            && $yearEqual['POSITION_2_ACCURACY'] >= Bt03e04Contract::POSITION_REDESIGN_P2_MIN_INCLUSIVE
            && $yearEqual['POSITION_3_ACCURACY'] > Bt03e04Contract::POSITION_REDESIGN_P3_MIN_EXCLUSIVE
            && $yearEqual['POSITION_HIT_RATE_AT_3'] > Bt03e04Contract::POSITION_REDESIGN_HIT3_MIN_EXCLUSIVE;
        $winPreservation = count(array_filter(
            $outer,
            static fn (array $result): bool => $result['delta']['WINNER_HIT_AT_1'] >= Bt03e04Contract::WIN_PRESERVATION_MIN_INCLUSIVE,
        )) === count($outer);
        $gates = [
            'integrity' => $integrity,
