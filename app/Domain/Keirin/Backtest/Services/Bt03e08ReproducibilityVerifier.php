...
final class Bt03e08ReproducibilityVerifier
{
    ...
    public function hash(array $result): string
    {
        ...
        foreach (['calculation_version', 'contract', 'source_bundle_identity', 'feature_source_integrity', 'outcome_snapshot_identity', 'fold_definitions', 'inner_layout_identities', 'outer_2024', 'outer_2025', 'prediction_manifests', 'paired_bootstrap_ci', 'acceptance_gate_input'] as $key) {
            if (! array_key_exists($key, $result)) {
                throw new RuntimeException("BT-03E-08 reproducibility payload lacked {$key}.");
            }
    ...
    public function verify(?string $previousArtifact, string $currentHash): array
    {
        if ($previousArtifact === null) {
            return ['status' => 'REPRODUCIBILITY VERIFICATION REQUIRED', 'verified' => false, 'previous_artifact' => null, 'previous_hash' => null, 'current_hash' => $currentHash];
        }
        if (! is_file($previousArtifact)) {
            throw new RuntimeException('BT-03E-08 reproducibility artifact did not exist.');
        }
        $size = filesize($previousArtifact);
        if ($size === false || $size < 1 || $size > self::MAX_ARTIFACT_BYTES) {
            throw new RuntimeException('BT-03E-08 reproducibility artifact size was invalid.');
        }
        ...
        if (! is_array($previous) || ! is_string($previous['reproducibility_hash'] ?? null)) {
            throw new RuntimeException('BT-03E-08 reproducibility artifact was invalid.');
        }
        ...
        if (! hash_equals($previousHash, $previous['reproducibility_hash']) || ! hash_equals($previousHash, $currentHash)) {
            throw new RuntimeException('BT-03E-08 reproducibility verification mismatched.');
        }

        return ['status' => 'VERIFIED', 'verified' => true, 'previous_artifact' => $previousArtifact, 'previous_hash' => $previousHash, 'current_hash' => $currentHash];
    }
}
