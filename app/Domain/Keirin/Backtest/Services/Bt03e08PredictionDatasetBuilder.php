...
final class Bt03e08PredictionDatasetBuilder
{
    ...
    public function __construct(
        ...
        private readonly Bt02SignalEligibilityEvaluator $signalEligibility, private readonly EffectBinBuilder $binBuilder,
    ...
    {
        if (! in_array($year, Bt03e08Contract::OUTER_YEARS, true)) {
            throw new RuntimeException('BT-03E-08 prediction year was outside 2024/2025.');
        }
        ...
        try {
            ...
            foreach ($source->races() as $race) {
                if (($race['year'] ?? null) !== $year) {
                    throw new RuntimeException('BT-03E-08 source prediction year drifted.');
                }
                $chunk[] = $race;
    ...
    private function appendChunk(Bt03e02RaceSpool $spool, int $year, array $races): void
    {
        ...
        foreach ($races as $sourceRace) {
            ...
            if ($baseline === [] || $sortedSource !== $sortedBaseline) {
                throw new RuntimeException("BT-03E-08 source E03 and current STAT-01 entrants differed for race {$raceId}.");
            }
            $signalMaps = [];
            foreach (Bt03e08Contract::STAT_CODES as $statCode) {
            ...
            foreach ($baseline as $feature) {
                ...
                foreach (Bt03e08Contract::STAT_CODES as $statCode) {
                    ...
                    $values[] = $signal !== null && $this->signalEligibility->eligible($statCode, Bt02SignalCohort::Operational, $signal) ? $signal->primaryValue : null;
                }
                $entriesByBike[$feature->bikeNumber] = ['id' => $feature->raceEntryId, 'bike' => $feature->bikeNumber, 'raw' => (float) $feature->raceScoreRaw, 'stat01_rank' => $feature->raceScoreRank, 'anchor' => $deviation > 0.0 ? ((float) $feature->raceScoreRaw - $mean) / $deviation : 0.0, 'anchor_status' => $deviation > 0.0 ? 'AVAILABLE' : 'ZERO_VARIANCE', 'signals' => $values, 'rank' => null, 'status' => 'PREDICTION_ONLY'];
            }
            $spool->append(['year' => $year, 'race_id' => $raceId, 'entries' => array_map(fn (int $bike): array => $entriesByBike[$bike], $sourceBikes)]);
        }
    }
