...
class Bt03e05DevelopmentEvaluationService
{
    /** @var list<Bt03e05RaceSpool> */
    private array $raceSpools = [];

    /** @var list<Bt03e05MetricContributionSpool> */
    private array $metricSpools = [];

    public function __construct(
        private readonly Bt03e05SourceBundleLoader $sourceBundles,
        private readonly Bt03eRuleSourceRepository $sourceRepository,
        private readonly Bt03eOutcomeSnapshotProvider $snapshots,
        private readonly Bt03e05BaselineSourcePreflightService $preflight,
        private readonly Bt03e02SourceIntegrityGuard $integrity,
        private readonly Bt03e05EvaluationContextLoader $contexts,
        private readonly Bt03e05DecisionDecoder $decoder,
        private readonly Bt03e05MetricEvaluator $metrics,
        private readonly Bt03e05PairedBootstrap $bootstrap,
        private readonly Bt03e05AcceptanceGate $acceptance,
        private readonly Bt03e05ReproducibilityVerifier $reproducibility,
        private readonly Bt03e05ArtifactWriter $artifacts,
        private readonly CanonicalHasher $hasher,
        private readonly Bt03e05ReadOnlyQueryAudit $queryAudit,
        private readonly Bt03e05OutcomeSnapshotEndVerifier $endSnapshotVerifier,
    ...
    public function run(
        ...
        $runIdentity = 'bt03e05-'.gmdate('Ymd-His').'-'.bin2hex(random_bytes(16));
        ...
        try {
            ...
            if (! hash_equals(Bt03SourceManifest::OUTCOME_SNAPSHOT_MANIFEST_HASH, $snapshot->manifestHash())) {
                throw new RuntimeException('BT-03E-05 outcome snapshot identity was invalid.');
            }
            ...
            foreach (Bt03e05Contract::DEVELOPMENT_YEARS as $year) {
                ...
                $decoderSpool = $this->trackRace(new Bt03e05RaceSpool(
                    'DECODER',
                    sys_get_temp_dir().'/bt03e05-decoder-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl',
                ));
                $metricSpool = $this->trackMetric(new Bt03e05MetricContributionSpool(
                    sys_get_temp_dir().'/bt03e05-metrics-'.$year.'-'.bin2hex(random_bytes(8)).'.bin',
                ));
                $manifest = new Bt03e05DecoderManifestAccumulator($this->hasher);
            ...
            $this->integrity->assertUnchanged($snapshotStart, $snapshotEnd, 'BT-03E-05 outcome snapshot');
            $baselineEnd = $this->preflight->run();
            $this->integrity->assertUnchanged($baselineStart, $baselineEnd, 'BT-03E-05 STAT-01 baseline source');
    ...
    private function evaluateYear(Bt03e05RaceSpool $source, Bt03e05RaceSpool $context, callable $consumer): void
    {
        ...
        while ($sourceIterator->valid() && $contextIterator->valid()) {
            ...
            if ($sourceRace['year'] !== $contextRace['year']
                ...
                throw new RuntimeException('BT-03E-05 source probability and evaluation context cohorts differed.');
            }
        ...
        if ($sourceIterator->valid() || $contextIterator->valid()) {
            throw new RuntimeException('BT-03E-05 source probability and evaluation context race counts differed.');
        }
    }
    ...
    private function assertSourceCompatibility(array $sourceResult, array $baseline, array $snapshot): void
    {
        ...
        if (! is_string($sourceSnapshotHash)
            ...
            throw new RuntimeException('BT-03E-05 source and evaluation outcome snapshots differed.');
        }
        $sourceRecords = $sourceResult['source_integrity']['start']['fingerprints'] ?? null;
        if (! is_array($sourceRecords)) {
            throw new RuntimeException('BT-03E-05 source baseline fingerprint evidence was unavailable.');
        }
        $sourceBaseline = array_values(array_filter(
            $sourceRecords,
            static fn (mixed $record): bool => is_array($record)
                && ($record['stat_code'] ?? null) === 'STAT-01'
                && in_array($record['year'] ?? null, Bt03e05Contract::DEVELOPMENT_YEARS, true),
        ));
        if ($sourceBaseline !== $baseline['fingerprints']) {
            throw new RuntimeException('BT-03E-05 source and evaluation STAT-01 fingerprints differed.');
        }
    }

    private function trackRace(Bt03e05RaceSpool $spool): Bt03e05RaceSpool
    {
        $this->raceSpools[] = $spool;
    ...
    private function trackMetric(Bt03e05MetricContributionSpool $spool): Bt03e05MetricContributionSpool
    {
