...
final class Bt03e08TrainingDatasetBuilder
{
    private const CHUNK_SIZE = 200;

    public function __construct(
        private readonly Bt01SourceManifest $baselineManifest, private readonly Bt02SourceManifest $signalManifest,
        private readonly BacktestFeatureRepository $baselineFeatures, private readonly Bt02SignalFeatureRepository $signalFeatures,
        private readonly FeatureEligibilityEvaluator $baselineEligibility, private readonly Bt02SignalEligibilityEvaluator $signalEligibility,
        private readonly Bt02LabelDefinition $labels, private readonly EffectBinBuilder $binBuilder,
    ...
    {
        if (! in_array($year, Bt03e08Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-08 dataset year was outside 2022-2025.');
        }
        $this->audit->recordSnapshotYear($year);
        $spool = new Bt03e02RaceSpool('RAW', $this->path($temporaryDirectory, "raw-{$year}"));
        $fold = new FoldDefinitionDto("BT03E08_{$year}", 0, null, null, new DateTimeImmutable("{$year}-01-01"), new DateTimeImmutable("{$year}-12-31"));
        try {
            foreach ($snapshot->chunks($fold, self::CHUNK_SIZE) as $chunk) {
                ...
                $signals = $this->signals($year, $raceIds);
    ...
    /** @param list<int> $raceIds @return array<string,array<int,list<object>>> */
    private function signals(int $year, array $raceIds): array
    {
        $signals = [];
        foreach (Bt03e08Contract::STAT_CODES as $statCode) {
            $this->audit->recordFeatureSourceYear($year);
            $signals[$statCode] = $this->signalFeatures->forRaces($this->signalManifest->for($year, $statCode)->featureRunId, $statCode, $raceIds);
        }

        return $signals;
    }

    /** @param array<int,list<object>> $baselineByRace @param array<string,array<int,list<object>>> $signals @return array<string,mixed>|null */
    private function rawRace(int $year, Bt02OutcomeContextRaceDto $snapshotRace, array $baselineByRace, array $signals): ?array
    {
        ...
        foreach ($snapshotRace->results as $result) {
            if (isset($results[$result->bikeNumber])) {
                throw new RuntimeException('BT-03E-08 outcome snapshot duplicated a bike number.');
            }
        ...
        if (count($results) !== $context->entrantCount || $baselineBikes !== $resultBikes) {
            throw new RuntimeException('BT-03E-08 outcome entrants did not match STAT-01 entrants.');
        }
        $signalMaps = [];
        foreach (Bt03e08Contract::STAT_CODES as $statCode) {
            foreach ($signals[$statCode][$context->raceId] ?? [] as $signal) {
                if (isset($signalMaps[$statCode][$signal->raceEntryId])) {
                    throw new RuntimeException("BT-03E-08 {$statCode} duplicated a race entry.");
                }
        ...
        foreach ($baseline as $feature) {
            ...
            foreach (Bt03e08Contract::STAT_CODES as $statCode) {
                ...
                $signalValues[] = $signal !== null && $this->signalEligibility->eligible($statCode, Bt02SignalCohort::Operational, $signal) ? $signal->primaryValue : null;
            }
            $result = $results[$feature->bikeNumber];
            $binary = $this->labels->labels($result->resultStatus, $result->rank);
            $entries[] = [
                'id' => $feature->raceEntryId, 'bike' => $feature->bikeNumber, 'raw' => (float) $feature->raceScoreRaw,
                'stat01_rank' => $feature->raceScoreRank, 'anchor' => $deviation > 0.0 ? ((float) $feature->raceScoreRaw - $mean) / $deviation : 0.0,
                'anchor_status' => $deviation > 0.0 ? 'AVAILABLE' : 'ZERO_VARIANCE', 'signals' => $signalValues,
                'labels' => [$binary->isWin, $binary->isTop2, $binary->isTop3], 'rank' => $result->rank, 'status' => $result->resultStatus,
            ];
        }
    ...
    {
        return rtrim($directory, '/').'/bt03e08-'.$role.'-'.bin2hex(random_bytes(8)).'.jsonl';
    }
}
