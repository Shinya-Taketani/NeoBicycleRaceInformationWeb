...
final class Bt03e08OutcomeEvaluationLoader
{
    private const CHUNK_SIZE = 200;

    public function __construct(private readonly Bt01SourceManifest $manifest, private readonly BacktestFeatureRepository $features, private readonly FeatureEligibilityEvaluator $eligibility, private readonly Bt03e08ReadOnlyQueryAudit $audit) {}
    ...
    public function build(int $year, Bt02OutcomeContextSnapshot $snapshot, string $temporaryDirectory = '/tmp'): Bt03e06RaceSpool
    {
        if (! in_array($year, Bt03e08Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-08 outcome year was outside 2022-2025.');
        }
        $spool = new Bt03e06RaceSpool('CONTEXT', rtrim($temporaryDirectory, '/').'/bt03e08-context-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl');
    ...
    /** @param array<int,list<object>> $baseline */
    private function race(int $year, Bt02OutcomeContextRaceDto $snapshotRace, array $baseline): ?array
    {
        ...
        foreach ($features as $feature) {
            ...
            $entries[] = ['id' => $feature->raceEntryId, 'bike' => $feature->bikeNumber, 'raw' => (float) $feature->raceScoreRaw, 'stat01_rank' => $feature->raceScoreRank, 'rank' => $result->rank, 'status' => $result->resultStatus];
        }
