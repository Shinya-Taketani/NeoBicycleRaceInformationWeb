...
final class Bt03e07SourcePreflightService
{
    ...
    public function run(): array
    {
        if ($this->signalManifest->computedHash() !== Bt02SourceManifest::HASH
            ...
            throw new RuntimeException('BT-03E-07 fixed source manifests were invalid.');
        }

        $baselineSources = array_map(
            fn (int $year): object => $this->baselineManifest->forYear($year),
            Bt03e07Contract::DEVELOPMENT_YEARS,
        );
        ...
        $records = [];
        $yearRecords = array_fill_keys(Bt03e07Contract::DEVELOPMENT_YEARS, []);
        foreach (Bt03e07Contract::DEVELOPMENT_YEARS as $year) {
    ...
    private function fingerprintRecord(
        ...
        if (! hash_equals($expectedSource, $source) || ! hash_equals($expectedContent, $content)) {
            throw new RuntimeException("BT-03E-07 source fingerprint drifted for {$year} {$statCode}.");
        }
    ...
    private function assertSignalMetadata(array $sources): array
    {
        ...
        foreach ($sources as $source) {
            ...
            if ($run === null || $count === null
                ...
                throw new RuntimeException("BT-03E-07 signal source {$source->featureRunId} drifted.");
            }
