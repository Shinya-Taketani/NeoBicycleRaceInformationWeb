...
final class Bt03e04EvaluationContextLoader
{
    ...
    public function build(int $year, Bt02OutcomeContextSnapshot $snapshot, string $temporaryDirectory = '/tmp'): Bt03e04RaceSpool
    {
        if (! in_array($year, Bt03e04Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-04 context year was outside 2024/2025.');
        }
        $spool = new Bt03e04RaceSpool(
            'CONTEXT',
            rtrim($temporaryDirectory, '/').'/bt03e04-context-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl',
        );
    ...
    private function race(int $year, Bt02OutcomeContextRaceDto $snapshotRace, array $baseline): ?array
    {
        ...
        foreach ($snapshotRace->results as $result) {
            if (isset($results[$result->bikeNumber])) {
                throw new RuntimeException('BT-03E-04 outcome snapshot duplicated a bike number.');
            }
        ...
        if (count($results) !== $context->entrantCount || $featureBikes !== $resultBikes) {
            throw new RuntimeException('BT-03E-04 outcome entrants did not match STAT-01 entrants.');
        }
        $entries = [];
