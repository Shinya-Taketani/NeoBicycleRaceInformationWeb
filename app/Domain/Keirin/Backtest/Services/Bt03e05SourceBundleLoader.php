...
final class Bt03e05SourceBundleLoader
{
    ...
    /** @return array{identity:array<string,mixed>,source_result:array<string,mixed>,years:array<int,Bt03e05RaceSpool>} */
    public function load(string $directory, string $temporaryDirectory = '/tmp'): array
    {
        if (! is_dir($directory)) {
            throw new RuntimeException('BT-03E-05 source bundle directory did not exist.');
        }
        $paths = [];
        foreach (['result.json', 'probabilities.csv', 'map_predictions.csv', 'manifest.json'] as $file) {
            ...
            if (! is_file($paths[$file])) {
                throw new RuntimeException("BT-03E-05 source bundle lacked {$file}.");
            }
        }
        ...
        try {
            foreach (Bt03e05Contract::DEVELOPMENT_YEARS as $year) {
                $spools[$year] = new Bt03e05RaceSpool(
                    'SOURCE',
                    rtrim($temporaryDirectory, '/').'/bt03e05-source-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl',
                );
            }
    ...
    private function validateManifest(array $manifest, array $paths): array
    {
        ...
        foreach ($manifest['files'] as $file) {
            if (! is_array($file) || array_keys($file) !== ['name', 'bytes', 'sha256']
                ...
                throw new RuntimeException('BT-03E-05 source artifact file manifest was invalid.');
            }
            ...
            if ($actualBytes !== $file['bytes'] || ! is_string($actualHash) || ! hash_equals($file['sha256'], $actualHash)) {
                throw new RuntimeException("BT-03E-05 source {$file['name']} bytes or SHA-256 mismatched.");
            }
            $files[$file['name']] = $file;
        }
        if (array_keys($files) !== ['result.json', 'probabilities.csv', 'map_predictions.csv']) {
            throw new RuntimeException('BT-03E-05 source artifact required exactly three manifested files in canonical order.');
        }
    ...
    private function validateResult(array $result): void
    {
        ...
        if (($result['calculation_version'] ?? null) !== Bt03e05Contract::SOURCE_CALCULATION_VERSION
            || ! is_array($contract)
            || ($contract['contract'] ?? null) !== Bt03e05Contract::SOURCE_CONTRACT_NAME
            || ($contract['calculation_version'] ?? null) !== Bt03e05Contract::SOURCE_CALCULATION_VERSION
            || ($contract['optimizer_version'] ?? null) !== Bt03e05Contract::SOURCE_OPTIMIZER_VERSION
            || ($contract['iteration_semantics_version'] ?? null) !== Bt03e05Contract::SOURCE_ITERATION_SEMANTICS_VERSION
            || ($contract['probability_version'] ?? null) !== Bt03e05Contract::SOURCE_PROBABILITY_VERSION
            || ($contract['artifact_version'] ?? null) !== Bt03e05Contract::SOURCE_ARTIFACT_VERSION
            || ($contract['prediction_manifest_version'] ?? null) !== Bt03e05Contract::SOURCE_PREDICTION_MANIFEST_VERSION) {
            throw new RuntimeException('BT-03E-05 rejected a non-v2 source model contract.');
        }
        foreach (Bt03e05Contract::DEVELOPMENT_YEARS as $year) {
            $outer = $result["outer_{$year}"] ?? null;
            if (! is_array($outer)
                || ($outer['model']['optimizer_version'] ?? null) !== Bt03e05Contract::SOURCE_OPTIMIZER_VERSION
                || ($outer['model']['probability_version'] ?? null) !== Bt03e05Contract::SOURCE_PROBABILITY_VERSION
                || ! is_array($outer['prediction_manifest'] ?? null)
                || ($outer['prediction_manifest']['version'] ?? null) !== Bt03e05Contract::SOURCE_PREDICTION_MANIFEST_VERSION
                ...
                throw new RuntimeException("BT-03E-05 source Outer {$year} prediction contract was invalid.");
            }
        }
        $verification = $result['reproducibility_verification'] ?? null;
        if (! is_array($verification) || ($verification['status'] ?? null) !== Bt03e05Contract::SOURCE_REPRODUCIBILITY_STATUS
            || ($verification['verified'] ?? null) !== true
            || ! is_string($result['reproducibility_hash'] ?? null)) {
            throw new RuntimeException('BT-03E-05 source reproducibility was not VERIFIED.');
        }
        ...
        if (! hash_equals($result['reproducibility_hash'], $actualReproducibility)
            ...
            throw new RuntimeException('BT-03E-05 source reproducibility hash was invalid.');
        }
        if (($result['acceptance_gate']['gates']['integrity'] ?? null) !== true
            ...
            throw new RuntimeException('BT-03E-05 source integrity was not PASS.');
        }
        if (($result['audit']['2026_access_count'] ?? null) !== 0
            ...
            throw new RuntimeException('BT-03E-05 source audit contained 2026 access.');
        }
    }

    /** @param array<int,Bt03e05RaceSpool> $spools @return array<int,array{race_count:int,entry_count:int}> */
    private function parseCsv(string $probabilityPath, string $mapPath, array $spools): array
    {
        ...
        if ($probabilities === false || $maps === false) {
            ...
            throw new RuntimeException('BT-03E-05 source CSV could not be opened.');
        }
        $counts = array_fill_keys(Bt03e05Contract::DEVELOPMENT_YEARS, ['race_count' => 0, 'entry_count' => 0]);
        $seen = [];
        try {
            if (fgetcsv($probabilities, escape: '') !== self::PROBABILITY_HEADER
                || fgetcsv($maps, escape: '') !== self::MAP_HEADER) {
                throw new RuntimeException('BT-03E-05 source CSV header was invalid.');
            }
            ...
            while ($probabilityRow !== null) {
                ...
                $key = "{$year}:{$raceId}";
                if (isset($seen[$key])) {
                    throw new RuntimeException('BT-03E-05 source probabilities duplicated a race.');
                }
                $seen[$key] = true;
                $entries = [];
                do {
                ...
                if ($mapRow === null || $this->identity($mapRow) !== [$year, $raceId]) {
                    throw new RuntimeException('BT-03E-05 source probability and MAP race streams differed.');
                }
            ...
            if ($mapRow !== null) {
                ...
                throw new RuntimeException('BT-03E-05 source MAP stream contained an extra race.');
            }
            if (! feof($probabilities) || ! feof($maps)) {
                throw new RuntimeException('BT-03E-05 source CSV streams did not end together.');
            }
        } finally {
            fclose($probabilities);
            fclose($maps);
    ...
    private function identity(array $row): array
    {
        ...
        if (! in_array($year, Bt03e05Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException($year === 2026
                ? 'BT-03E-05 source CSV contained forbidden 2026 data.'
                : 'BT-03E-05 source CSV year was outside 2024/2025.');
        }
    ...
    private function probabilityEntry(array $row): array
    {
        if (count($row) !== count(self::PROBABILITY_HEADER)) {
            throw new RuntimeException('BT-03E-05 probability CSV row width was invalid.');
        }
        ...
        return [
            ...
            'source_is_map_top3' => match ($row[9]) {
                '0' => false,
                '1' => true,
                default => throw new RuntimeException('BT-03E-05 source is_map_top3 was invalid.'),
            },
        ];
    }
    ...
    private function map(array $row, array $entrants): array
    {
        if (count($row) !== count(self::MAP_HEADER)) {
            throw new RuntimeException('BT-03E-05 MAP CSV row width was invalid.');
        }
    ...
    private function assertProbabilityRace(array $entries): void
    {
        if (count($entries) < 5 || count($entries) > 9) {
            throw new RuntimeException('BT-03E-05 source entrant count was outside 5-9.');
        }
        ...
        if (count(array_unique($bikes)) !== count($bikes)
            ...
            throw new RuntimeException('BT-03E-05 source bike numbers were invalid.');
        }
        foreach ([1, 2, 3] as $position) {
            $sum = array_sum(array_column($entries, "position_{$position}_probability"));
            if (abs($sum - 1.0) > Bt03e05Contract::PROBABILITY_TOLERANCE) {
                throw new RuntimeException("BT-03E-05 source P{$position} sum invariant failed.");
            }
        }
        foreach ($entries as $entry) {
            $top2 = $entry['position_1_probability'] + $entry['position_2_probability'];
            $top3 = $top2 + $entry['position_3_probability'];
            if (abs($entry['top2_probability'] - $top2) > Bt03e05Contract::PROBABILITY_TOLERANCE
                || abs($entry['top3_probability'] - $top3) > Bt03e05Contract::PROBABILITY_TOLERANCE
                || $entry['top2_probability'] > 1.0 + Bt03e05Contract::PROBABILITY_TOLERANCE
                || $entry['top3_probability'] > 1.0 + Bt03e05Contract::PROBABILITY_TOLERANCE) {
                throw new RuntimeException('BT-03E-05 source marginal probability invariant failed.');
            }
        }
    }
    ...
    private function validatePredictionCounts(array $result, array $counts): void
    {
        foreach (Bt03e05Contract::DEVELOPMENT_YEARS as $year) {
            ...
            if ($counts[$year]['race_count'] !== $manifest['race_count']
                ...
                throw new RuntimeException("BT-03E-05 source Outer {$year} CSV counts mismatched its prediction manifest.");
            }
        }
    }
    ...
    private function csvRow($handle): ?array
    {
        ...
        if ($row === [null]) {
            throw new RuntimeException('BT-03E-05 source CSV contained an empty row.');
        }
    ...
    private function integer(?string $value, string $field): int
    {
        if ($value === null || preg_match('/\A[1-9]\d*\z/', $value) !== 1) {
            throw new RuntimeException("BT-03E-05 source {$field} was invalid.");
        }
    ...
    private function probability(?string $value, string $field): float
    {
        ...
        if (! is_finite($probability) || $probability < 0.0 || $probability > 1.0) {
            throw new RuntimeException("BT-03E-05 source {$field} was outside [0,1].");
        }
    ...
    private function bikeTriple(?string $value, string $field, array $entrants): array
    {
        ...
        if (count(array_unique($bikes)) !== 3 || array_diff($bikes, $entrants) !== []) {
            throw new RuntimeException("BT-03E-05 source {$field} entrants were invalid.");
        }
    ...
    private function jsonFile(string $path): array
    {
        ...
        if ($size === false || $size < 1 || $size > self::MAX_JSON_BYTES) {
            throw new RuntimeException('BT-03E-05 source JSON size was invalid.');
        }
        $contents = file_get_contents($path);
        try {
            $value = is_string($contents) ? json_decode($contents, true, flags: JSON_THROW_ON_ERROR) : null;
        } catch (JsonException $exception) {
            throw new RuntimeException('BT-03E-05 source JSON was invalid.', previous: $exception);
        }
        if (! is_array($value) || array_is_list($value)) {
            throw new RuntimeException('BT-03E-05 source JSON root was invalid.');
        }
