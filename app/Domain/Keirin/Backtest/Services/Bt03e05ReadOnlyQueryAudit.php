...
final class Bt03e05ReadOnlyQueryAudit
{
    ...
    {
        if ($this->active) {
            throw new RuntimeException('BT-03E-05 query audit was already active.');
        }
        $this->active = true;
        ...
        DB::listen(function (QueryExecuted $query): void {
            ...
            if (preg_match('/\A(?:insert|update|delete|merge|truncate|alter|create|drop)\b/i', ltrim($query->sql)) === 1) {
                $this->writeCount++;
                throw new RuntimeException('BT-03E-05 blocked a database write statement.');
            }
            ...
            if (preg_match('/(?<!\d)2026-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])(?:[T ][^\s,"\']*)?/', $audit) === 1) {
                ...
                throw new RuntimeException('BT-03E-05 blocked 2026 query access.');
            }
        });
    }
    ...
    public function recordDecoderContractFrozen(): void
    {
        if (! $this->active || $this->decoderFrozen) {
            throw new RuntimeException('BT-03E-05 decoder contract freeze order was invalid.');
        }
    ...
    public function recordSourceBundleValidated(): void
    {
        if (! $this->active || ! $this->decoderFrozen || $this->sourceValidated) {
            throw new RuntimeException('BT-03E-05 source bundle validation order was invalid.');
        }
    ...
    {
        if (! $this->active) {
            throw new RuntimeException('BT-03E-05 query audit was not active.');
        }
        $this->active = false;
        if (! $this->decoderFrozen || ! $this->sourceValidated || $this->writeCount !== 0 || $this->forbidden2026Access !== 0
            ...
            || $this->baselineAccess[2024] < 1 || $this->baselineAccess[2025] < 1) {
            throw new RuntimeException('BT-03E-05 read-only, temporal, or 2026 audit failed.');
        }
    ...
    {
        if (! $this->active || ! $this->decoderFrozen || ! $this->sourceValidated
            || ! in_array($year, Bt03e05Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException("BT-03E-05 {$role} year access was forbidden or invalid.");
        }
        $target[$year]++;
