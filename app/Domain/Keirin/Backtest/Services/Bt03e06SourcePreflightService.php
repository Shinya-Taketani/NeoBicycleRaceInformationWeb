...
final class Bt03e06SourcePreflightService
{
    ...
    public function run(): array
    {
        if ($this->signalManifest->computedHash() !== Bt02SourceManifest::HASH
            ...
            throw new RuntimeException('BT-03E-06 fixed source manifests were invalid.');
        }

        $baselineSources = array_map(
            fn (int $year): object => $this->baselineManifest->forYear($year),
            Bt03e06Contract::DEVELOPMENT_YEARS,
        );
        ...
        $signalMetadata = $this->assertSignalMetadata($signalSources);
        $this->fingerprints->assertVersionContract();

        $records = [];
        $yearRecords = array_fill_keys(Bt03e06Contract::DEVELOPMENT_YEARS, []);
        foreach (Bt03e06Contract::DEVELOPMENT_YEARS as $year) {
            $manifest = $this->baselineManifest->forYear($year);
            $fingerprint = $this->baselineFingerprints->forYear($year);
            $record = $this->fingerprintRecord(
                $year,
                'STAT-01',
                $fingerprint->featureRunId,
                $fingerprint->sourceFingerprintSha256,
                $fingerprint->contentFingerprintSha256,
                [
                    'feature_run_uuid' => $manifest->featureRunUuid,
                    'calculation_version' => $fingerprint->calculationVersion,
                    'target_from' => $manifest->targetFrom,
                    'target_to' => $manifest->targetTo,
                    'race_count' => $manifest->expectedRaceCount,
                    'row_count' => $manifest->expectedResultCount,
        ...
        foreach ($signalSources as $source) {
            $record = $this->fingerprintRecord(
                ...
                $source->contentFingerprintSha256,
                [
                    ...
                    'race_count' => $source->processedRaceCount,
                    'row_count' => $source->rowCount,
                    'status' => $signalMetadata[$source->featureRunId]['status'],
                    'error_count' => $signalMetadata[$source->featureRunId]['error_count'],
                ],
            );
        ...
        foreach ($yearRecords as $year => $yearRecord) {
            $yearDigests[$year] = $this->hasher->hash($yearRecord);
        }
        ...
        return [
            ...
            'year_fingerprint_digests' => $yearDigests,
    ...
    private function fingerprintRecord(
        int $year,
        string $statCode,
        int $runId,
        ...
        if (! hash_equals($expectedSource, $source) || ! hash_equals($expectedContent, $content)) {
            throw new RuntimeException("BT-03E-06 source fingerprint drifted for {$year} {$statCode}.");
        }
    ...
    private function assertSignalMetadata(array $sources): array
    {
        ...
        foreach ($sources as $source) {
            ...
            if ($run === null || $count === null
                ...
                throw new RuntimeException("BT-03E-06 signal source {$source->featureRunId} drifted.");
            }
            $metadata[$source->featureRunId] = [
