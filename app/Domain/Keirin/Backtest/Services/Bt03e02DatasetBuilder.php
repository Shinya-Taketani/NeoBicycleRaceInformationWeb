...
final class Bt03e02DatasetBuilder
{
    ...
    private function rawRace(int $year, Bt02OutcomeContextRaceDto $snapshotRace, array $baselineByRace, array $signals): ?array
    {
        ...
        foreach ($snapshotRace->results as $result) {
            if (isset($results[$result->bikeNumber])) {
                throw new RuntimeException('BT-03E-02 outcome snapshot duplicated a bike number.');
            }
        ...
        if (count($results) !== $context->entrantCount || $baselineBikes !== $resultBikes) {
            throw new RuntimeException('BT-03E-02 outcome entrants did not match STAT-01 entrants.');
        }
        $signalMaps = [];
        foreach (Bt03e02Contract::STAT_CODES as $statCode) {
            foreach ($signals[$statCode][$context->raceId] ?? [] as $signal) {
                if (isset($signalMaps[$statCode][$signal->raceEntryId])) {
                    throw new RuntimeException("BT-03E-02 {$statCode} duplicated a race entry.");
                }
    ...
    {
        if (! in_array($year, Bt03e02Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-02 dataset year was outside 2022-2025.');
        }
    }

    private function path(string $directory, string $role): string
    {
        return rtrim($directory, '/').'/bt03e02-'.$role.'-'.bin2hex(random_bytes(8)).'.jsonl';
    }
}
