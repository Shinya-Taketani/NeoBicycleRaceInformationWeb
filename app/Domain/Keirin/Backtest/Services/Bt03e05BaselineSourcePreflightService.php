...
final class Bt03e05BaselineSourcePreflightService
{
    ...
    public function run(): array
    {
        if ($this->fingerprintManifest->computedHash() !== Bt02BaselineFingerprintManifest::HASH) {
            throw new RuntimeException('BT-03E-05 fixed baseline fingerprint manifest was invalid.');
        }
        $sources = array_map(fn (int $year) => $this->manifest->forYear($year), Bt03e05Contract::DEVELOPMENT_YEARS);
        ...
        foreach (Bt03e05Contract::DEVELOPMENT_YEARS as $year) {
            ...
            if (! hash_equals($expected->sourceFingerprintSha256, $source)
                ...
                throw new RuntimeException("BT-03E-05 STAT-01 source fingerprint drifted for {$year}.");
            }
            $records[] = [
                'year' => $year,
