...
final class Bt03e04ReproducibilityVerifier
{
    ...
    public function hash(array $result): string
    {
        ...
        foreach ([
            ...
            if (! array_key_exists($key, $result)) {
                throw new RuntimeException("BT-03E-04 reproducibility payload lacked {$key}.");
            }
    ...
    public function verify(?string $previousArtifact, string $currentHash): array
    {
        ...
        if (! is_file($previousArtifact)) {
            throw new RuntimeException('BT-03E-04 reproducibility artifact did not exist.');
        }
        $size = filesize($previousArtifact);
        if ($size === false || $size < 1 || $size > self::MAX_ARTIFACT_BYTES) {
            throw new RuntimeException('BT-03E-04 reproducibility artifact size was invalid.');
        }
        ...
        if (! is_array($previous) || ! is_string($previous['reproducibility_hash'] ?? null)) {
            throw new RuntimeException('BT-03E-04 reproducibility artifact was invalid.');
        }
        $previousHash = $this->hash($previous);
        if (! hash_equals($previousHash, $previous['reproducibility_hash'])) {
            throw new RuntimeException('BT-03E-04 previous artifact reproducibility hash was invalid.');
        }
        if (! hash_equals($previousHash, $currentHash)) {
            throw new RuntimeException('BT-03E-04 reproducibility verification mismatched.');
        }
