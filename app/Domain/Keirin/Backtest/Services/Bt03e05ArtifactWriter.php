...
final class Bt03e05ArtifactWriter
{
    ...
    /** @param array<string,mixed> $summary @param array<int,Bt03e05RaceSpool> $decoders @return array<string,string> */
    public function write(string $directory, array $summary, array $decoders): array
    {
        ...
        $name = 'bt03e05-development-'.gmdate('Ymd-His').'-'.bin2hex(random_bytes(16));
        $temporary = $root.'/.'.$name.'.tmp';
        $final = $root.'/'.$name;
        if ($this->filesystem->exists($temporary) || $this->filesystem->exists($final)) {
            throw new RuntimeException('BT-03E-05 artifact bundle already existed.');
        }
        ...
        try {
            ...
            if (! is_string($reproducibilityHash) || preg_match('/\A[a-f0-9]{64}\z/', $reproducibilityHash) !== 1) {
                throw new RuntimeException('BT-03E-05 reproducibility hash was invalid.');
            }
            ...
            try {
                $this->filesystem->writeCsvRow($handle, [
                    ...
                    'primary_position_3_bike', 'primary_position_1_probability', 'primary_position_2_probability',
                    'primary_position_3_probability', 'primary_second_third_objective_score',
                    'map_ordered_top3', 'map_ordered_probability',
                ...
                if (array_keys($decoders) !== Bt03e05Contract::DEVELOPMENT_YEARS) {
                    throw new RuntimeException('BT-03E-05 artifact required exactly 2024/2025 decoder streams.');
                }
                foreach ($decoders as $year => $spool) {
                    ...
                    foreach ($spool->races() as $decision) {
                        ...
                        $this->filesystem->writeCsvRow($handle, [
                            ...
                            sprintf('%.17g', $decision['primary_position_1_probability']),
                            sprintf('%.17g', $decision['primary_position_2_probability']),
                            sprintf('%.17g', $decision['primary_position_3_probability']),
                    ...
                    if (! is_array($expected) || $actual !== $expected) {
                        throw new RuntimeException("BT-03E-05 {$year} decoder semantic manifest mismatched its artifact stream.");
                    }
                }
            ...
            foreach (['result.json' => $resultPath, 'decoder_predictions.csv' => $decoderPath] as $name => $path) {
                ...
                if ($hash === false) {
                    throw new RuntimeException("BT-03E-05 {$name} hash could not be calculated.");
                }
                $files[] = ['name' => $name, 'bytes' => $this->filesystem->size($path), 'sha256' => $hash];
            }
            $manifest = ['artifact_version' => Bt03e05Contract::ARTIFACT_VERSION, 'files' => $files];
