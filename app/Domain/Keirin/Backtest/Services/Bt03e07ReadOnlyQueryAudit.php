...
final class Bt03e07ReadOnlyQueryAudit
{
    ...
    {
        if ($this->active) {
            throw new RuntimeException('BT-03E-07 query audit was already active.');
        }
        $this->active = true;
        ...
        $this->candidateManifestSealed = array_fill_keys(Bt03e07Contract::OUTER_YEARS, false);
        ...
        DB::listen(function (QueryExecuted $query): void {
            ...
            if (preg_match('/\A(?:insert|update|delete|merge|truncate|alter|create|drop)\b/i', ltrim($query->sql)) === 1) {
                $this->writeCount++;
                throw new RuntimeException('BT-03E-07 blocked a database write statement.');
            }
            ...
            if (preg_match('/(?<!\d)2026-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01])(?:[T ][^\s,"\']*)?/', $evidence) === 1) {
                ...
                throw new RuntimeException('BT-03E-07 blocked 2026 query access.');
            }
        });
    }
    ...
    public function recordContractFrozen(): void
    {
        if (! $this->active || $this->contractFrozen) {
            throw new RuntimeException('BT-03E-07 contract freeze order was invalid.');
        }
    ...
    public function recordSourceBundleValidated(): void
    {
        if (! $this->active || ! $this->contractFrozen || $this->sourceValidated) {
            throw new RuntimeException('BT-03E-07 source validation order was invalid.');
        }
    ...
    {
        if (! $this->ready() || ! in_array($year, Bt03e07Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-07 feature year access was forbidden or invalid.');
        }
    ...
    {
        if (! $this->ready() || ! in_array($year, Bt03e07Contract::OUTER_YEARS, true)
            || $this->candidateManifestSealed[$year] || $this->featureAccess[$year] < 1) {
            throw new RuntimeException('BT-03E-07 candidate manifest seal order was invalid.');
        }
    ...
    public function recordSnapshotYear(int $year): void
    {
        if (! $this->ready() || ! in_array($year, Bt03e07Contract::DEVELOPMENT_YEARS, true)
            ...
            throw new RuntimeException("BT-03E-07 {$year} outcome access occurred before its candidate manifest seal.");
        }
    ...
    {
        if (! $this->active) {
            throw new RuntimeException('BT-03E-07 query audit was not active.');
        }
        $this->active = false;
        if (! $this->contractFrozen || ! $this->sourceValidated
            ...
            || $this->featureAccess[2026] !== 0 || $this->snapshotAccess[2026] !== 0
            || min(array_intersect_key($this->featureAccess, array_flip(Bt03e07Contract::DEVELOPMENT_YEARS))) < 1
            || min(array_intersect_key($this->snapshotAccess, array_flip(Bt03e07Contract::DEVELOPMENT_YEARS))) < 1) {
            throw new RuntimeException('BT-03E-07 read-only, temporal, or 2026 audit failed.');
        }
