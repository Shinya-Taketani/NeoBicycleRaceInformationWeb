...
final class Bt03e07OutcomeEvaluationLoader
{
    ...
    public function build(int $year, Bt02OutcomeContextSnapshot $snapshot, string $temporaryDirectory = '/tmp'): Bt03e06RaceSpool
    {
        if (! in_array($year, Bt03e07Contract::DEVELOPMENT_YEARS, true)) {
            throw new RuntimeException('BT-03E-07 outcome year was outside 2024/2025.');
        }
        $spool = new Bt03e06RaceSpool(
            'CONTEXT',
            rtrim($temporaryDirectory, '/').'/bt03e07-context-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl',
        );
