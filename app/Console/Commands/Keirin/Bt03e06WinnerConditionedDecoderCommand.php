...
final class Bt03e06WinnerConditionedDecoderCommand extends Command
{
    protected $signature = 'keirin:backtest:bt03e06
        {--plan : Display the frozen winner-conditioned decoder contract without source or database access}
        {--execute : Execute the 2024/2025 development evaluation}
        {--source-bundle= : Verified BT-03E-03 v2 artifact bundle}
        {--verify-reproducibility= : Previous BT-03E-06 result.json}
        {--output-dir=/tmp : Non-Git directory for immutable execution artifacts}';

    protected $description = 'Plan or execute the BT-03E-06 winner-conditioned sequential decoder evaluation.';

    public function handle(Bt03e06DevelopmentEvaluationService $service): int
    {
        ...
        if ($plan) {
            ...
            $this->info('BT-03E-06 PLAN');
            foreach ([
                'contract', 'calculation_version', 'decoder_version', 'forward_reconstruction_version',
                'reconstruction_manifest_version', 'primary_tie_rule_version', 'supporting_tie_rule_version',
                'winner_rule', 'q2_formula', 'q3_marginal_formula', 'pair_objective',
            ] as $key) {
        ...
        $this->info('BT-03E-06 DEVELOPMENT EVALUATION COMPLETE');
