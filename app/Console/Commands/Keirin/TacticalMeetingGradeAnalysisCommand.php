<?php

declare(strict_types=1);

namespace App\Console\Commands\Keirin;

use App\Domain\Keirin\Backtest\Experiments\TacticalMeetingGradeAnalysis\Contract;
use App\Domain\Keirin\Backtest\Experiments\TacticalMeetingGradeAnalysis\Service;
use Illuminate\Console\Command;
use RuntimeException;
use Throwable;

final class TacticalMeetingGradeAnalysisCommand extends Command
{
    protected $signature = 'keirin:backtest:tactical-meeting-grade-analysis {--plan} {--execute} {--reproduce}
        {--source-bundle=} {--output-root=} {--analysis-id=}';

    protected $description = 'Analyze fixed Outer C1 decisions by historical meeting grade, without inference.';

    public function handle(): int
    {
        try {
            if (count(array_filter([$this->option('plan'), $this->option('execute'), $this->option('reproduce')])) !== 1) {
                throw new RuntimeException('Specify exactly one operation.');
            }
            $result = $this->option('plan') ? Contract::plan() : ($this->option('execute')
                ? app(Service::class)->execute($this->required('output-root'), $this->required('analysis-id'), $this->required('source-bundle'))
                : app(Service::class)->reproduce($this->required('output-root'), $this->required('analysis-id')));
            $this->line(json_encode($result, JSON_THROW_ON_ERROR | JSON_PRESERVE_ZERO_FRACTION | JSON_UNESCAPED_SLASHES));

            return self::SUCCESS;
        } catch (Throwable $e) {
            $this->error($e->getMessage());

            return self::FAILURE;
        }
    }

    private function required(string $name): string
    {
        $value = $this->option($name);
        if (! is_string($value) || $value === '') {
            throw new RuntimeException('Explicit --'.$name.' is required.');
        }

        return $value;
    }
}
