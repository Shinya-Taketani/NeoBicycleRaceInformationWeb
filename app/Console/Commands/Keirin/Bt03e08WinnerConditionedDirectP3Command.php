...
final class Bt03e08WinnerConditionedDirectP3Command extends Command
{
    protected $signature = 'keirin:backtest:bt03e08
        {--plan : Display the frozen P1/Q2 and winner-conditioned direct P3 contract}
        {--execute : Execute the 2024/2025 development evaluation}
        {--source-bundle= : Verified BT-03E-03 v2 artifact bundle}
        {--verify-reproducibility= : Previous BT-03E-08 result.json}
        {--output-dir=/tmp : Non-Git directory for immutable execution artifacts}';

    protected $description = 'Plan or execute the BT-03E-08 P1/Q2-frozen winner-conditioned direct P3 evaluation.';

    public function handle(Bt03e08DevelopmentEvaluationService $service): int
    {
        ...
        if ($plan) {
            $contract = Bt03e08Contract::plan();
            $this->info('BT-03E-08 PLAN');
            foreach (['contract', 'calculation_version', 'objective_version', 'optimizer_version', 'lambda_selection_version', 'probability_version', 'decoder_version', 'p1_freeze_rule', 'q2_freeze_rule', 'p3_training_candidate_set', 'p3_prediction_formula', 'pair_objective', 'primary_tie_rule_version', 'supporting_tie_rule_version'] as $key) {
        ...
        $this->info('BT-03E-08 DEVELOPMENT EVALUATION COMPLETE');
