...
`synthetic/race-sync-jsj017-race-day-terminated.json` is a minimized synthetic reproduction of the JSJ017 response saved by Batch 111 for the third meeting day on 2021-08-29. It preserves the exact termination message and the missing `keirinCd`, `kaisaihi`, and `rInfo` structure needed for regression coverage. Real meeting names and encrypted parameters are not retained.
...
`synthetic/race-sync-pj0301-meeting-cancelled-populated-races.html` is a minimized and anonymized synthetic reproduction of the PJ0301 cancellation shape saved by Batch 97 for four meetings in 2022. It retains the three-day schedule and 12 populated race rows whose parameters are present, races are ended, and results are available, while all meeting names and encrypted parameters are fictional. Tests also reduce this fixture to nine and three rows to verify that cancellation recognition is structural rather than tied to observed race counts.

`synthetic/race-sync-pj0301-strict-postponed-meeting.html` is a minimal synthetic reproduction based on public PJ0301 responses observed for first-day postponements at Sasebo and Nara in August 2023. Real names and encrypted parameters are not retained. It preserves `flgRaceCancel=true`, `flgSectionCancel=false`, the postponement message, zero `cntRace`, four meeting dates, nine race metadata rows, and the repeated first-day label.

`synthetic/race-sync-jsj001-partial-race-cancelled.json` is a minimal synthetic reproduction based on the public JSJ001 structure observed for Maebashi on 2024-03-31, where only race 11 was cancelled. Meeting names and encrypted parameters are fictional; the three meeting days, 11 available race parameters, selected race 11, section-cancellation flag, zero `cntRace`, and race-specific cancellation message are retained.
