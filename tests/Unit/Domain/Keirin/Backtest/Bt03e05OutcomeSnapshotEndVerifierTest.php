...
use App\Domain\Keirin\Backtest\Services\Bt03e05OutcomeSnapshotEndVerifier;
...
class Bt03e05OutcomeSnapshotEndVerifierTest extends TestCase
{
    public function test_reopened_end_snapshot_verifies_only_2024_and_2025_before_returning_its_identity(): void
    {
        ...
        try {
            $endSnapshot = Bt02OutcomeContextSnapshotArtifact::open($directory, 'synthetic/bt03e05/outcome');
            $end = (new Bt03e05OutcomeSnapshotEndVerifier)->verify($endSnapshot, $audit);
    ...
    public function test_partition_drift_after_evaluation_read_fails_before_artifact_publication(): void
    {
        ...
        try {
            ...
            try {
                $endSnapshot = Bt02OutcomeContextSnapshotArtifact::open($directory, 'synthetic/bt03e05/outcome');
                (new Bt03e05OutcomeSnapshotEndVerifier)->verify($endSnapshot, $audit);
            ...
            $servicePath = (new ReflectionClass(Bt03e05DevelopmentEvaluationService::class))->getFileName();
            ...
            if ($audit->active()) {
                try {
                    $audit->finish();
                    $this->fail('An incomplete end verification must not pass the final audit.');
                } catch (RuntimeException $exception) {
                    $this->assertStringContainsString('temporal', $exception->getMessage());
                }
            }
            $this->remove($directory);
        }
    ...
    private function startedAudit(): Bt03e05ReadOnlyQueryAudit
    {
    ...
    {
        $directory = sys_get_temp_dir().'/bt03e05-end-snapshot-'.bin2hex(random_bytes(8));
        ...
        return [$directory, Bt02OutcomeContextSnapshotArtifact::open($directory, 'synthetic/bt03e05/outcome')];
    }
