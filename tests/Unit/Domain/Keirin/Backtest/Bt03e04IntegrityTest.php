...
use App\Domain\Keirin\Backtest\Services\Bt03e04ArtifactWriter;
...
class Bt03e04IntegrityTest extends TestCase
{
    public function test_contract_freezes_versions_decoders_bootstrap_and_2026_guard(): void
    {
        $plan = Bt03e04Contract::plan();

        $this->assertSame('BT03E04-DECODER-SEPARATION-v1', $plan['calculation_version']);
        $this->assertSame('BT03E04-DECODERS-v1', $plan['decoder_version']);
        $this->assertSame('BT03E04-DECODER-TIE-v1', $plan['tie_rule_version']);
        $this->assertSame('BT03E04-DEVELOPMENT-ARTIFACT-v1', $plan['artifact_version']);
        $this->assertSame('BT03E04-DECODER-SEMANTIC-MANIFEST-v1', $plan['decoder_manifest_version']);
    ...
    public function test_bootstrap_is_paired_year_equal_and_bit_exact_deterministic(): void
    {
        ...
        try {
            $bootstrap = new Bt03e04PairedBootstrap(new Type7Quantile);
            $first = $bootstrap->evaluate($spools);
            $second = $bootstrap->evaluate($spools);

            $this->assertSame($first, $second);
            $this->assertSame(array_keys(array_fill_keys(Bt03e04MetricEvaluator::METRIC_CODES, null)), array_keys($first));
        } finally {
    ...
    public function test_gate_uses_coherent_primary_thresholds_unchanged(): void
    {
        [$outer, $intervals] = $this->passingGateInput();
        $passed = (new Bt03e04AcceptanceGate)->evaluate($outer, $intervals, true);

        $this->assertSame('PASS / GO_TO_FREEZE', $passed['status']);
        $intervals['WINNER_HIT_AT_1']['ci_lower'] = -0.0015;
        $failed = (new Bt03e04AcceptanceGate)->evaluate($outer, $intervals, true);
    ...
    {
        $gate = new Bt03e04AcceptanceGate;
        [$outer, $intervals] = $this->passingGateInput();
    ...
    {
        $verifier = new Bt03e04ReproducibilityVerifier(new CanonicalHasher);
        $base = $this->summary();
        $changedRuntime = $base;
    ...
    public function test_reproducibility_requires_a_second_matching_result_and_fails_on_semantic_drift(): void
    {
        ...
        $path = sys_get_temp_dir().'/bt03e04-repro-'.bin2hex(random_bytes(8)).'.json';
    ...
    {
        $directory = sys_get_temp_dir().'/bt03e04-artifact-'.bin2hex(random_bytes(8));
        ...
        try {
            ...
            $summary['reproducibility_hash'] = (new Bt03e04ReproducibilityVerifier(new CanonicalHasher))->hash($summary);
            $paths = (new Bt03e04ArtifactWriter(new Bt03eArtifactFilesystem, new CanonicalHasher))->write($directory, $summary, $spools);
    ...
    {
        $directory = sys_get_temp_dir().'/bt03e04-artifact-fail-'.bin2hex(random_bytes(8));
        mkdir($directory, 0775, true);
        $spools = [2024 => $this->decoderSpool(2024), 2025 => $this->decoderSpool(2025)];
        try {
            $summary = $this->summary();
            $summary['reproducibility_hash'] = (new Bt03e04ReproducibilityVerifier(new CanonicalHasher))->hash($summary);
            try {
                (new Bt03e04ArtifactWriter(new Bt03eArtifactFilesystem, new CanonicalHasher))->write($directory, $summary, $spools);
    ...
    public function test_artifact_write_and_atomic_publish_failures_leave_no_partial_bundle(): void
    {
        foreach (['write', 'publish'] as $failure) {
            $directory = sys_get_temp_dir().'/bt03e04-artifact-'.$failure.'-'.bin2hex(random_bytes(8));
            mkdir($directory, 0775, true);
            $spools = [2024 => $this->decoderSpool(2024), 2025 => $this->decoderSpool(2025)];
            try {
                $summary = $this->summary();
                ...
                try {
                    (new Bt03e04ArtifactWriter($filesystem, new CanonicalHasher))->write($directory, $summary, $spools);
    ...
    {
        $constructor = (new ReflectionClass(Bt03e04DevelopmentEvaluationService::class))->getConstructor();
        $types = array_map(
            static fn (\ReflectionParameter $parameter): string => (string) $parameter->getType(),
            $constructor?->getParameters() ?? [],
        );
        $source = (string) file_get_contents((new ReflectionClass(Bt03e04DevelopmentEvaluationService::class))->getFileName());
    ...
    private function metricSpool(int $year): Bt03e04MetricContributionSpool
    {
        $spool = new Bt03e04MetricContributionSpool(sys_get_temp_dir().'/bt03e04-bootstrap-'.$year.'-'.bin2hex(random_bytes(8)).'.bin');
    ...
    private function passingGateInput(): array
    {
        ...
        foreach ([2024, 2025] as $year) {
            $outer[$year] = [
                'delta' => array_fill_keys(Bt03e04MetricEvaluator::METRIC_CODES, 0.01),
                'tie_diagnostics' => [
        ...
        return [
            $outer,
            array_fill_keys(Bt03e04MetricEvaluator::METRIC_CODES, ['ci_lower' => 0.001, 'ci_upper' => 0.02]),
        ];
    }

    private function decoderSpool(int $year): Bt03e04RaceSpool
    {
        $spool = new Bt03e04RaceSpool('DECODER', sys_get_temp_dir().'/bt03e04-decoder-'.$year.'-'.bin2hex(random_bytes(8)).'.jsonl');
        $source = $this->uniformSource($year);
        $spool->append((new Bt03e04DecisionDecoder)->decode($source));
        $spool->seal();
    ...
    {
        $manifest = ['version' => Bt03e04Contract::DECODER_MANIFEST_VERSION, 'race_count' => 1, 'semantic_sha256' => str_repeat('a', 64)];
