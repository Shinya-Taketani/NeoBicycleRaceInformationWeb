...
class Bt03e05MetricsTest extends TestCase
{
    public function test_primary_and_supporting_metric_decoder_mapping_is_applied(): void
    {
        $source = $this->source();
        $decision = (new Bt03e05DecisionDecoder)->decode($source);
        $comparison = (new Bt03e05MetricEvaluator)->raceComparison($this->context(), $decision);

        $this->assertSame(1.0, $comparison['candidate']['WINNER_HIT_AT_1']['numerator']);
        $this->assertSame(1.0, $comparison['diagnostics']['PRIMARY_WINNER_HIT_AT_1']);
    ...
    public function test_stat01_baseline_matches_e04_for_normal_and_dead_heat(array $ranks): void
    {
        $context = $this->context($ranks);
        $decision = (new Bt03e05DecisionDecoder)->decode($this->source());
        $e05 = (new Bt03e05MetricEvaluator)->raceComparison($context, $decision);
        $e04Decision = (new Bt03e04DecisionDecoder)->decode($this->source());
        $e04 = (new Bt03e04MetricEvaluator)->raceComparison($context, $e04Decision);

        $this->assertSame($e04['baseline'], $e05['baseline']);
        $this->assertSame($e04['ordered_eligible'], $e05['ordered_eligible']);
    }
