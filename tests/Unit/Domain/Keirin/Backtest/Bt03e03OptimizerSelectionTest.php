...
class Bt03e03OptimizerSelectionTest extends TestCase
{
    public function test_regularization_path_is_strong_to_weak_and_warm_started(): void
    {
        ...
        $this->assertSame(Bt03e03Contract::FIT_EXECUTION_ORDER, $path['fit_order']);
        $previous = null;
    ...
    public function test_candidate_is_available_only_when_all_positions_converge(): void
    {
        ...
        foreach ($path['fits'] as $key => $fit) {
            ...
            $this->assertSame(Bt03e03Contract::POSITIONS, array_keys($fit->coefficients));
            $this->assertSame(Bt03e03Contract::POSITIONS, array_keys($fit->diagnostics));
        }
    ...
    public function test_one_se_uses_position_equal_and_year_equal_loss(): void
    {
        $first = $this->spool([
            '0.01' => [1.0, 2.0, 3.0],
            '0.10000000000000001' => [3.0, 3.0, 3.0],
        ], 1);
        $second = $this->spool([
            '0.01' => [5.0, 5.0, 5.0],
            '0.10000000000000001' => [3.0, 3.0, 3.0],
        ...
        try {
            $selection = (new Bt03e03OneSeSelector)->select([2023 => $first, 2024 => $second], 20);
    ...
    {
        $spool = $this->spool(['0.10000000000000001' => [0.5, 0.5, 0.5]], 2);
        try {
            $selection = (new Bt03e03OneSeSelector)->select([2023 => $spool], 20);
        ...
        try {
            ...
            (new Bt03e03OneSeSelector)->select([2023 => $empty], 20);
        } finally {
            $empty->cleanup();
        }
    }

    private function optimizer(): Bt03e03FistaOptimizer
    {
        return new Bt03e03FistaOptimizer(new Bt03e03ConditionalSoftmaxObjective);
    }
    ...
    private function spool(array $losses, int $raceCount): Bt03e03ValidationLossSpool
    {
        $spool = new Bt03e03ValidationLossSpool(
            sys_get_temp_dir().'/bt03e03-selection-'.bin2hex(random_bytes(8)).'.bin',
            array_keys($losses),
        );
