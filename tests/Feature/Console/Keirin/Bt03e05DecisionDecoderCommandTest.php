...
class Bt03e05DecisionDecoderCommandTest extends TestCase
{
    public function test_plan_needs_neither_source_bundle_nor_database_execution(): void
    {
        $this->mock(Bt03e05DevelopmentEvaluationService::class, function (MockInterface $mock): void {
            $mock->shouldNotReceive('run');
        });

        $this->artisan('keirin:backtest:bt03e05 --plan')
    ...
    {
        $this->artisan('keirin:backtest:bt03e05')->assertExitCode(1);
        $this->artisan('keirin:backtest:bt03e05 --plan --execute')->assertExitCode(1);
        $this->artisan('keirin:backtest:bt03e05 --execute')->assertExitCode(1);
        $this->artisan('keirin:backtest:bt03e05 --plan --source-bundle=/tmp/source')->assertExitCode(1);
    }
    ...
    public function test_execute_passes_the_required_fixed_source_bundle(): void
    {
        ...
        $this->mock(Bt03e05DevelopmentEvaluationService::class, function (MockInterface $mock) use ($metrics): void {
        ...
        $this->artisan('keirin:backtest:bt03e05 --execute --source-bundle=/tmp/source')
            ->expectsOutputToContain('BT-03E-05 DEVELOPMENT EVALUATION COMPLETE')
