...
class SyncAutomatedRacesCommandTest extends TestCase
{
    ...
    public function test_strict_postponed_meeting_syncs_three_days_and_is_idempotent(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->strictPostponedMeetingWithDays();
        $this->fakeStrictPostponedMeetingResponses();
        $arguments = [
            '--from' => '2024-08-01',
            '--to' => '2024-08-04',
            '--track-code' => '99',
        ...
        $this->artisan('keirin:races:sync-race-list', $arguments)
            ->expectsOutputToContain('days=3 races=27 entries=180 unresolved_players=180 skipped=1 failed=0')
            ->assertExitCode(0);
        ...
        $this->assertSame(
            [
                'enc-strict-postponed-day-1',
                'enc-strict-postponed-day-2',
                'enc-strict-postponed-day-3',
                'enc-strict-postponed-day-4',
            ],
            $days->pluck('encrypted_parameter')->all(),
        );
        $this->assertSame(0, Race::query()->where('race_day_id', $days[0]->id)->count());
        foreach ($days->slice(1) as $day) {
            $this->assertSame(9, Race::query()->where('race_day_id', $day->id)->count());
        }
        $this->assertSame(27, Race::query()->count());
        $this->assertSame(180, RaceEntry::query()->count());
        $raceIds = Race::query()->orderBy('race_date')->orderBy('race_number')->pluck('id')->all();
        $entryIds = RaceEntry::query()->orderBy('race_id')->orderBy('bike_number')->pluck('id')->all();
        ...
        $this->assertSame(
            $raceIds,
            Race::query()->orderBy('race_date')->orderBy('race_number')->pluck('id')->all(),
        );
        $this->assertSame(
            $entryIds,
            RaceEntry::query()->orderBy('race_id')->orderBy('bike_number')->pluck('id')->all(),
        );
        ...
        $this->assertSame(2, BatchRunItem::query()
            ->where('item_type', 'RACE_DAY_METADATA')
            ->where('status', 'SUCCEEDED')
            ->count());
        $this->assertSame(2, BatchRunItem::query()
            ->where('item_type', 'RACE_ENTRY_LIST')
            ->where('status', 'SKIPPED')
            ->where('skip_reason', 'RACE_DAY_POSTPONED')
            ->count());
        $this->assertSame(6, BatchRunItem::query()
            ->where('item_type', 'RACE_ENTRY_LIST')
            ->where('status', 'SUCCEEDED')
            ->count());
    ...
    public function test_cancelled_race_meeting_is_skipped_audited_and_idempotent_without_daily_processing(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->cancelledMeetingWithDays();
        $meetingBefore = $meeting->refresh()->toArray();
        $daysBefore = RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray();
        $response = $this->fixture('race-sync-pj0301-meeting-cancelled.html');
        $this->mock(RaceListConsistencyValidator::class, function (MockInterface $mock): void {
            $mock->shouldNotReceive('validate');
        });
        $this->partialMock(RaceRepository::class, function (MockInterface $mock): void {
            $mock->shouldNotReceive('updateMeetingDayParameters');
            $mock->shouldNotReceive('syncRaceDay');
        });
        $this->fakeRaceListResponses(raceListResponse: $response);
        $arguments = [
            '--from' => '2025-12-03',
            '--to' => '2025-12-05',
            '--sleep-ms' => '1',
        ...
        for ($attempt = 0; $attempt < 2; $attempt++) {
            $this->artisan('keirin:races:sync-race-list', $arguments)
                ->expectsOutputToContain('days=0 races=0 entries=0 unresolved_players=0 skipped=3 failed=0')
                ->assertExitCode(0);
        }

        $this->assertSame($meetingBefore, $meeting->refresh()->toArray());
        $this->assertSame($daysBefore, RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray());
        $this->assertSame(0, Race::query()->count());
        $this->assertSame(0, RaceEntry::query()->count());
        Http::assertSentCount(2);
        $this->assertTrue(Http::recorded()->every(
            fn (array $pair): bool => str_contains($pair[0]->url(), '/pc/racelist'),
        ));

        $items = BatchRunItem::query()->where('item_type', 'RACE_DAY_METADATA')->orderBy('id')->get();
        $this->assertCount(2, $items);
        foreach ($items as $item) {
            $this->assertSame('SKIPPED', $item->status);
            $this->assertSame('RACE_MEETING_CANCELLED', $item->skip_reason);
            $this->assertSame((int) $meeting->id, $item->metadata['race_meeting_id']);
            $this->assertSame($meeting->external_meeting_id, $item->metadata['external_meeting_id']);
            $this->assertSame('2025-12-03', $item->metadata['starts_on']);
            $this->assertSame('2025-12-05', $item->metadata['ends_on']);
            $this->assertSame(3, $item->metadata['target_day_count']);
            $this->assertSame('RACE_MEETING_CANCELLED', $item->metadata['reason']);
            $this->assertSame('中止となりました。', $item->metadata['message']);
            $this->assertSame(['20251203', '20251204', '20251205'], $item->metadata['evidence']['raceDates']);
            $this->assertNotEmpty($item->metadata['raw_file_path']);
        }

        $logs = ScrapingFetchLog::query()->where('request_url', 'like', '%/pc/racelist%')->orderBy('id')->get();
        $this->assertCount(2, $logs);
        foreach ($logs as $log) {
            $this->assertSame($response, Storage::disk('local')->get($log->raw_file_path));
        }
        $this->assertSame(2, BatchRun::query()->where('status', 'SUCCEEDED')->whereNotNull('finished_at')->count());
        $this->assertSame(0, BatchRun::query()->where('status', 'RUNNING')->count());
    }

    public function test_cancelled_race_meeting_with_populated_races_is_strictly_skipped_and_audited(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->cancelledMeetingWithDays();
        $meetingBefore = $meeting->refresh()->toArray();
        $daysBefore = RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray();
        $response = $this->fixture('race-sync-pj0301-meeting-cancelled-populated-races.html');
        $this->mock(RaceListConsistencyValidator::class, function (MockInterface $mock): void {
            $mock->shouldNotReceive('validate');
        });
        $this->partialMock(RaceRepository::class, function (MockInterface $mock): void {
        ...
        $this->assertSame(0, RacePayout::query()->count());
        $this->assertSame(0, BatchRunItem::query()->where('item_type', 'RACE_ENTRY_LIST')->count());
        Http::assertSentCount(2);
        $this->assertTrue(Http::recorded()->every(
            fn (array $pair): bool => str_contains($pair[0]->url(), '/pc/racelist'),
        ));
        ...
        foreach ($items as $item) {
            ...
            $this->assertSame(1, $item->attempt_count);
            $this->assertSame(3, $item->metadata['target_day_count']);
            $this->assertSame('populated_array', $item->metadata['evidence']['raceInfoState']);
            $this->assertSame(12, $item->metadata['evidence']['raceInfoCount']);
            $this->assertTrue($item->metadata['evidence']['allRaceParametersPresent']);
            $this->assertTrue($item->metadata['evidence']['allRacesEnded']);
            $this->assertTrue($item->metadata['evidence']['allResultsAvailable']);
            $this->assertNotEmpty($item->metadata['raw_file_path']);
        }
    ...
    public function test_cancelled_race_meeting_preserves_existing_races_entries_results_and_payouts(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->cancelledMeetingWithDays();
        $day = RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('race_date')->firstOrFail();
        $player = Player::query()->create([
            'source' => 'keirin_jp',
            'external_player_id' => 'meeting-cancelled-player',
            'name' => '既存選手',
        ]);
        $race = Race::query()->create([
            'source' => 'keirin_jp',
            'external_race_id' => '25:20251203:01',
            'race_day_id' => $day->id,
            'racetrack_id' => $meeting->racetrack_id,
            'race_date' => '2025-12-03',
            'race_number' => 1,
            'name' => '既存開催中止レース',
            'race_type' => 'A級予選',
            'entrant_count' => 5,
        ...
        foreach (range(1, 5) as $bikeNumber) {
            RaceEntry::query()->create([
                'race_id' => $race->id,
                'player_id' => $bikeNumber === 1 ? $player->id : null,
        ...
        RaceResult::query()->create([
            ...
            'rank' => 1,
            'result_status' => 'FINISHED',
            'raw_result_text' => 'existing meeting-cancelled result',
            'source_url' => 'https://example.test/existing-result',
            'fetched_at' => now(),
        ]);
        RacePayout::query()->create([
        ...
        $before = [
            'meeting' => $meeting->refresh()->toArray(),
            'days' => RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray(),
        ...
        $this->fakeRaceListResponses(raceListResponse: $this->fixture('race-sync-pj0301-meeting-cancelled-populated-races.html'));
        $arguments = ['--from' => '2025-12-03', '--to' => '2025-12-05', '--sleep-ms' => '1'];

        $this->artisan('keirin:races:sync-race-list', $arguments)->assertExitCode(0);
        $this->artisan('keirin:races:sync-race-list', $arguments)->assertExitCode(0);

        $this->assertSame($before['meeting'], $meeting->refresh()->toArray());
        $this->assertSame($before['days'], RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray());
        $this->assertSame($before['players'], Player::query()->orderBy('id')->get()->toArray());
        $this->assertSame($before['races'], Race::query()->orderBy('id')->get()->toArray());
        $this->assertSame($before['entries'], RaceEntry::query()->orderBy('id')->get()->toArray());
        $this->assertSame($before['results'], RaceResult::query()->orderBy('id')->get()->toArray());
        $this->assertSame($before['payouts'], RacePayout::query()->orderBy('id')->get()->toArray());
        $this->assertSame('CONFIRMED', $race->refresh()->result_status);
        $this->assertSame(2, BatchRunItem::query()->where('skip_reason', 'RACE_MEETING_CANCELLED')->count());
        $this->assertSame(0, BatchRun::query()->where('status', 'RUNNING')->count());
    }

    public function test_cancelled_race_meeting_context_mismatches_fail_without_changing_data(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->cancelledMeetingWithDays();
        $meetingBefore = $meeting->refresh()->toArray();
        $daysBefore = RaceDay::query()->where('race_meeting_id', $meeting->id)->orderBy('id')->get()->toArray();
        $fixture = (new EmbeddedJsonExtractor)->extract(
            $this->fixture('race-sync-pj0301-meeting-cancelled-populated-races.html'),
            'PC0201',
        );
        $cases = [
            'different track' => function (array &$json): void {
                $json['C0201data']['selKjyoCd'] = '26';
            },
            'different selected date' => function (array &$json): void {
                $json['C0201data']['selKaisai'] = '20251204';
            },
            'different date set' => function (array &$json): void {
                $json['C0201data']['C0201kaisai'][0]['txtEventDate'] = '12/02';
            },
            'different date order' => function (array &$json): void {
                [$json['C0201data']['C0201kaisai'][0], $json['C0201data']['C0201kaisai'][1]] = [
                    $json['C0201data']['C0201kaisai'][1],
                    $json['C0201data']['C0201kaisai'][0],
                ];
            },
        ];
        ...
        foreach ($cases as $case => $mutate) {
            ...
            $mutate($json);
            $this->fakeRaceListResponses(
                raceListResponse: json_encode($json, JSON_THROW_ON_ERROR | JSON_UNESCAPED_UNICODE),
            );
            $exitCode = Artisan::call('keirin:races:sync-race-list', [
                '--from' => '2025-12-03',
                '--to' => '2025-12-05',
            ...
            $this->assertSame(1, $exitCode, $case.': '.$output);
            $this->assertStringContainsString('days=0 races=0 entries=0 unresolved_players=0 skipped=0 failed=3', $output);
            $item = BatchRunItem::query()->latest('id')->firstOrFail();
            $this->assertSame('FAILED', $item->status);
            $this->assertSame('RACE_MEETING_CANCELLED', $item->metadata['reason']);
            $this->assertNotEmpty($item->metadata['raw_file_path']);
            $this->assertStringContainsString('did not match the stored race meeting', (string) $item->error_message);
        }
    ...
    public function test_sync_continues_with_the_next_meeting_after_a_cancelled_meeting(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $cancelledMeeting = $this->cancelledMeetingWithDays();
        $normalMeeting = $this->meetingWithDays();
        Http::fake(function (Request $request) use ($cancelledMeeting) {
            if (str_contains($request->url(), '/pc/racelist')) {
                parse_str($request->body(), $form);
                $response = ($form['encp'] ?? null) === $cancelledMeeting->encrypted_parameter
                    ? $this->fixture('race-sync-pj0301-meeting-cancelled.html')
                    : $this->fixture('race-sync-racelist.html');

                return Http::response($response, 200, ['Content-Type' => 'text/html; charset=UTF-8']);
            }
        ...
        $this->artisan('keirin:races:sync-race-list', [
            ...
            '--to' => '2026-06-16',
            '--sleep-ms' => '1',
        ])->expectsOutputToContain('days=1 races=12 entries=78 unresolved_players=78 skipped=3 failed=0')
            ->assertExitCode(0);

        $this->assertSame(0, Race::query()->whereIn(
            'race_day_id',
            RaceDay::query()->where('race_meeting_id', $cancelledMeeting->id)->select('id'),
        )->count());
        $this->assertSame(12, Race::query()->whereIn(
            'race_day_id',
            RaceDay::query()->where('race_meeting_id', $normalMeeting->id)->select('id'),
        )->count());
        $this->assertDatabaseHas('batch_run_items', [
            'item_type' => 'RACE_DAY_METADATA',
            'item_key' => 'race-meeting:'.$cancelledMeeting->id,
            'status' => 'SKIPPED',
            'skip_reason' => 'RACE_MEETING_CANCELLED',
        ]);
        $this->assertSame(0, BatchRun::query()->where('status', 'RUNNING')->count());
    }

    public function test_race_specific_cancellation_syncs_all_eleven_races_and_entries_idempotently(): void
    {
        Storage::fake('local');
        config(['keirin.sleep_ms' => 0, 'keirin.retry_times' => 0]);
        $meeting = $this->partialCancellationMeetingWithDays();
        $day = RaceDay::query()
            ->where('race_meeting_id', $meeting->id)
            ->whereDate('race_date', '2024-03-31')
            ->firstOrFail();
        $metadataResponse = $this->fixture('race-sync-jsj001-partial-race-cancelled.json');
        $entryResponse = $this->partialRaceCancelledEntries();
        $entries = json_decode($entryResponse, true, flags: JSON_THROW_ON_ERROR);
        $existingRace = Race::query()->create([
            'source' => 'keirin_jp',
            'external_race_id' => '22:20240331:11',
            'race_day_id' => $day->id,
            'racetrack_id' => $meeting->racetrack_id,
            ...
            'race_type' => '既存11R',
            'entrant_count' => 1,
            'encrypted_parameter' => 'existing-r11',
            'result_available' => false,
        ]);
        $firstEntrant = $entries['rInfo'][10]['sInfo'][0];
        $existingEntry = RaceEntry::query()->create([
            'race_id' => $existingRace->id,
            'bike_number' => (int) $firstEntrant['syaban'],
            'external_player_id' => $firstEntrant['senNo'],
            'fetched_at' => now(),
        ]);
        $this->fakeRaceListResponses(
            entryResponse: $entryResponse,
            metadataResponse: $metadataResponse,
            raceListResponse: $metadataResponse,
        );
        $arguments = [
            '--date' => '2024-03-31',
            '--sleep-ms' => '1',
        ...
        $this->artisan('keirin:races:sync-race-list', $arguments)
            ->expectsOutputToContain('days=1 races=11 entries=72 unresolved_players=72 skipped=0 failed=0')
            ->assertExitCode(0);

        $this->assertSame(range(1, 11), Race::query()->where('race_day_id', $day->id)->orderBy('race_number')->pluck('race_number')->all());
        $race11 = Race::query()->where('external_race_id', '22:20240331:11')->firstOrFail();
        $this->assertSame((int) $existingRace->id, (int) $race11->id);
        $this->assertSame('enc-partial-r11', $race11->encrypted_parameter);
        $this->assertTrue($race11->result_available);
        $this->assertSame(6, RaceEntry::query()->where('race_id', $race11->id)->count());
        $this->assertSame((int) $existingEntry->id, (int) RaceEntry::query()
            ->where('race_id', $race11->id)
            ->where('bike_number', $existingEntry->bike_number)
            ->value('id'));
        $this->assertSame(72, RaceEntry::query()->whereIn(
            'race_id',
            Race::query()->where('race_day_id', $day->id)->select('id'),
        )->count());
        $raceIds = Race::query()->where('race_day_id', $day->id)->orderBy('race_number')->pluck('id')->all();
        $entryIds = RaceEntry::query()->whereIn('race_id', $raceIds)->orderBy('race_id')->orderBy('bike_number')->pluck('id')->all();

        $this->artisan('keirin:races:sync-race-list', $arguments)
            ->expectsOutputToContain('days=1 races=11 entries=72 unresolved_players=72 skipped=0 failed=0')
            ->assertExitCode(0);

        $this->assertSame($raceIds, Race::query()->where('race_day_id', $day->id)->orderBy('race_number')->pluck('id')->all());
        $this->assertSame(
            $entryIds,
            RaceEntry::query()->whereIn('race_id', $raceIds)->orderBy('race_id')->orderBy('bike_number')->pluck('id')->all(),
        );
        $items = BatchRunItem::query()->where('item_type', 'RACE_ENTRY_LIST')->orderBy('id')->get();
        $this->assertCount(2, $items);
        foreach ($items as $item) {
            $this->assertSame('SUCCEEDED', $item->status);
            $this->assertNull($item->skip_reason);
            $this->assertSame(11, $item->metadata['races']);
            $this->assertSame(72, $item->metadata['entries']);
        }

        $metadataLogs = ScrapingFetchLog::query()->where('request_url', 'like', '%type=JSJ001%')->get();
        $entryLogs = ScrapingFetchLog::query()->where('request_url', 'like', '%type=JSJ017%')->get();
        $raceListLogs = ScrapingFetchLog::query()->where('request_url', 'like', '%/pc/racelist%')->get();
        $this->assertCount(2, $metadataLogs);
        $this->assertCount(2, $entryLogs);
        $this->assertCount(2, $raceListLogs);
        foreach ($metadataLogs->merge($raceListLogs) as $log) {
            $this->assertSame($metadataResponse, Storage::disk('local')->get($log->raw_file_path));
        }
        foreach ($entryLogs as $log) {
            $this->assertSame($entryResponse, Storage::disk('local')->get($log->raw_file_path));
        }
        $this->assertSame(2, BatchRun::query()->where('status', 'SUCCEEDED')->whereNotNull('finished_at')->count());
        $this->assertSame(0, BatchRun::query()->where('status', 'RUNNING')->count());
    }
    ...
    {
        $track = Racetrack::query()->create(['source' => 'keirin_jp', 'external_track_id' => '25', 'name' => '合成中止競輪場']);
        $meeting = RaceMeeting::query()->create([
            'source' => 'keirin_jp',
            'external_meeting_id' => '25:20251203:synthetic-cancelled',
            'racetrack_id' => $track->id,
            'meeting_name' => '合成中止開催',
            'starts_on' => '2025-12-03',
            'ends_on' => '2025-12-05',
            'duration_days' => 3,
            'encrypted_parameter' => 'enc-cancelled-meeting',
        ]);
        foreach (range(1, 3) as $day) {
            RaceDay::query()->create([
                'race_meeting_id' => $meeting->id,
                'external_race_day_id' => "synthetic-cancelled-day-{$day}",
                'race_date' => '2025-12-'.str_pad((string) ($day + 2), 2, '0', STR_PAD_LEFT),
                'day_number' => $day,
            ]);
        }
    ...
    private function strictPostponedMeetingWithDays(): RaceMeeting
    {
        $track = Racetrack::query()->create(['source' => 'keirin_jp', 'external_track_id' => '99', 'name' => '合成順延競輪場']);
        $meeting = RaceMeeting::query()->create([
            'source' => 'keirin_jp',
            'external_meeting_id' => '99:20240801:synthetic-strict-postponed',
            'racetrack_id' => $track->id,
            'meeting_name' => '合成順延開催',
            'starts_on' => '2024-08-01',
            'ends_on' => '2024-08-04',
            'duration_days' => 4,
            'encrypted_parameter' => 'enc-strict-postponed-meeting',
        ]);
        foreach (range(1, 4) as $day) {
            RaceDay::query()->create([
                'race_meeting_id' => $meeting->id,
                'external_race_day_id' => "synthetic-strict-postponed-day-{$day}",
                'race_date' => '2024-08-'.str_pad((string) $day, 2, '0', STR_PAD_LEFT),
                'day_number' => $day,
                'encrypted_parameter' => "enc-strict-postponed-day-{$day}",
            ]);
        }
    ...
    private function partialCancellationMeetingWithDays(): RaceMeeting
    {
        $track = Racetrack::query()->create(['source' => 'keirin_jp', 'external_track_id' => '22', 'name' => '合成部分中止競輪場']);
        $meeting = RaceMeeting::query()->create([
            'source' => 'keirin_jp',
            'external_meeting_id' => '22:20240329:synthetic-partial-cancellation',
            'racetrack_id' => $track->id,
            'meeting_name' => '合成部分中止開催',
            'starts_on' => '2024-03-29',
            'ends_on' => '2024-03-31',
            'duration_days' => 3,
            'encrypted_parameter' => 'enc-partial-meeting',
        ]);
        foreach (range(1, 3) as $day) {
            RaceDay::query()->create([
                'race_meeting_id' => $meeting->id,
                'external_race_day_id' => "synthetic-partial-cancellation-day-{$day}",
                'race_date' => '2024-03-'.str_pad((string) ($day + 28), 2, '0', STR_PAD_LEFT),
                'day_number' => $day,
            ]);
        }
    ...
    private function fakeRaceListResponses(
        ?string $entryResponse = null,
        ?string $metadataResponse = null,
        ?string $raceListResponse = null,
    ): void {
        Http::fake(function (Request $request) use ($entryResponse, $metadataResponse, $raceListResponse) {
            if (str_contains($request->url(), '/pc/racelist')) {
                return Http::response($raceListResponse ?? $this->fixture('race-sync-racelist.html'), 200, ['Content-Type' => 'text/html; charset=UTF-8']);
            }
    ...
    private function fakeStrictPostponedMeetingResponses(): void
    {
        Http::fake(function (Request $request) {
            if (str_contains($request->url(), '/pc/racelist')) {
                return Http::response(
                    $this->fixture('race-sync-pj0301-strict-postponed-meeting.html'),
                    200,
            ...
            $dayByParameter = [
                'enc-strict-postponed-day-1' => '20240801',
                'enc-strict-postponed-day-2' => '20240802',
                'enc-strict-postponed-day-3' => '20240803',
                'enc-strict-postponed-day-4' => '20240804',
            ];
            $raceDate = $dayByParameter[$query['encp'] ?? ''] ?? null;

            if (($query['type'] ?? null) === 'JSJ001') {
                $metadata = (new EmbeddedJsonExtractor)->extract(
                    $this->fixture('race-sync-pj0301-strict-postponed-meeting.html'),
                    'PC0201',
                );
                $metadata['C0201data']['selKaisai'] = $raceDate;
                if ($raceDate !== '20240801') {
                    $metadata['C0201data']['hhMessage'] = null;
                    $metadata['C0201data']['flgRaceCancel'] = false;
                    $metadata['C0201data']['cntRace'] = 9;
                }
            ...
            if ($raceDate === '20240801') {
                return Http::response(
                    $this->fixture('race-sync-jsj017-postponed.json'),
                    200,
            ...
            $entries = json_decode($this->fixture('race-sync-jsj017.json'), true, flags: JSON_THROW_ON_ERROR);
            $entries['keirinCd'] = '99';
            $entries['kaisaihi'] = $raceDate;
            $entries['reqprm']['bkcd'] = '99';
            $entries['reqprm']['kday'] = $raceDate;
            $entries['rInfo'] = array_slice($entries['rInfo'], 0, 9);
